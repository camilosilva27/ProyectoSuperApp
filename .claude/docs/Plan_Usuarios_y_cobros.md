...
**Verificado en vivo (2026-09-11)**: se bajó `plan` a `'gratis'` a mano en la cuenta real del usuario (mismo patrón ya usado para probar el paywall de fin de trial: mutación temporal reversible en una fila real) y se corrió `verificarYAplicarPago` — el pago permanente se re-detectó vía `payments/search` por `external_reference`, `plan` volvió a `'premium'` solo, y **no se mandó un recibo duplicado** (confirmado contra los eventos de Brevo: `pagado_en` no había cambiado, así que la guarda de idempotencia de `procesarPagoAprobado` no disparó `enviarRecibo`).
