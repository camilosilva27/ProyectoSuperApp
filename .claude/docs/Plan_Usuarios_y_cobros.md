...
**Pendiente, no bloqueante**: no se agregó todavía un cron de reintento para quien paga y no vuelve a abrir la app — si hace falta, es el próximo paso natural (mismo patrón que los crons de `backend/src/cron/`, reintentando `procesarPagoAprobado`/`procesarSuscripcion` para usuarios con un pago iniciado pero sin confirmar en las últimas horas).
