...
- **MODO + banco → exige las dos** (`resolverTarjetasPromo`, campo `requiereTodas`): "ICBC Modo", "Banco Credicoop MODO", "Hipotecario_Modo", "Banco_Comafi_MODO" (Chango Más) y en Coto "Pagando con MODO desde la app de Comafi/Supervielle/Ciudad", "desde APP BANCA CREDICOOP con MODO" (el banco puede salir solo del ícono, ej. Ciudad) quedaban como "MODO o el banco" — un usuario con solo MODO recibía el 30% de Comafi o el ICBC 20% de un jueves sin MODO genérico. Ahora `canonicosPosibles = [banco, 'MODO']` + `requiereTodas: true`, y el usuario tiene que tener marcadas TODAS: único criterio `promoAplicaATarjetas()`, usado por `filtrarPromosBancariasPorTarjetas` (comparar/CLI), la grilla (`promosBancariasGrilla.js`, logo = el del banco) y `misDescuentos.js` (se lista bajo el banco, no bajo MODO). `bancoCanonico` se muestra como "ICBC (MODO)". "X MODO" con banco sin canónico ("Yoy_MODO") se descarta. "Galicia Modo" sigue siendo su propio canónico (ya no matchea además "MODO" genérico).
...
- **No se tocó (decisiones de negocio pendientes):** niveles de una misma tarjeta (Naranja X Inicial/Turbo/Épico, Patagonia Clásica/Plus/Singular), segmentos (jubilados, plan sueldo, ICBC Sueldos, Supervielle Identité, Mi Carrefour ANSES/+60), "no acumulables" con la promo de producto, promos limitadas a categorías, MasGO exclusivas, Billeteras Virtuales, "10% todos los medios online" de Carrefour, Comunidad Coto, topes mensuales tratados como por ticket. Tampoco "Modo|BNA" de Día (dos nombres en la misma entrada: el modelo "cualquiera de / todas" no expresa "MODO y BNA" si vienen separados; hoy queda como MODO genérico).
