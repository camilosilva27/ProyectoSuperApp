...
**Endpoint de promos:** `POST /_v/search-promotions` con `{ seller, skus: [skuId, ...] }`. Máximo 10 SKUs por request. Devuelve `promotions.generic.promotions[skuId].{ name, code, categoryType, value, effectiveDiscount, start, end }`.

**`categoryType` manda, no `effectiveDiscount` (bug real corregido 2026-09-24).** Tipos vistos en datos reales:
- `"percentual"`, `"nxm"`, `"segundo_al"`: `effectiveDiscount` es correcto y el `name` trae "35% …"/"3x2 …"/"2do al 50% …" — los interpreta `interpretarPromoPorTexto` como siempre.
- **`"fixed_price"`** (`name` "OFERTA TRAVIATA | Ofertas Trafico", `code` "Oferta"): el precio real de la oferta es **`value`** (precio UNITARIO en $, ej. 790). `effectiveDiscount` es un número de la promo ENTERA (el mismo para todos los SKUs de la campaña, redondeado a 2 decimales), no del SKU. Antes se usaba como % directo: Traviata (EAN 7790040143944, $790 real) salía Vea $769,50 / Disco $810 / Jumbo $850,50; pelador Krea (skuId 412145) $2.383 en vez de $1.490. Con los catálogos de prod de ese día, de los SKUs fixed_price con error >2%: Jumbo 193/223, Disco 166/196, Vea 9/186 → 0 después del fix. Ahora el `promocion` del catálogo guarda `categoryType` y `value` (y `descuento`/`descuentoPct`/`precioFinal` son el % y precio REALES sobre `precioBase`), y el motor lo lee como `oferta_precio_fijo` con `nUnidades: 1`, `precioFijoTotal: value`. Si `value` falta o es `>= precioBase` (visto: soda Cuisine & Co en Disco, base $1.100 y "oferta" $1.480) → sin promo, no se inventa. Las 257 fixed_price vistas eran todas "OFERTA X" (ninguna "2x$" ni por N unidades).
- `"Llevando n x"` (`code` "Llevando 6", `value` 0, sin `effectiveDiscount`): no dice ni % ni precio → se descarta (sin promo), no se adivina.
- Existe también un grupo `promotions.sgc` (ej. "SGC Promotion precio fijo") además de `generic`: no se usa (solo se lee `generic`), a propósito.
- Lógica única en `AllPromos/core/promoCencosud.js` (`promocionCatalogoCencosud` para los 9 scrapers/completadores/refrescadores de Vea/Jumbo/Disco, `promoMotorCencosud` para `precioCache.entradasCencosud` y el fallback en vivo). Catálogos viejos sin `categoryType` se leen igual que antes (siguen con el error hasta la próxima corrida del cron). Tests: `AllPromos/core/cencosud-precio-fijo.test.js`.
...
- `commertialOffer.Teasers[]` / `PromotionTeasers[]` → **siempre vacíos en Chango Más** (0 teasers en ~3600 SKUs revisados el 10/08 y el 19/08, y 0 apariciones de "masclub" en `commertialOffer`). El parseo por simetría con Carrefour sigue en los scripts, pero no aporta nada.
- **Promos por cantidad (2x1, "2da al X%", "2x$precio", "3x2", "3X$precio") — SÍ existen, pero SOLO en la simulación de checkout (descubierto 2026-09-24).** Antes esta doc decía que las promos condicionales de Chango Más "no se vieron nunca": era porque se buscaban en el catálogo. Aparecen en `POST https://www.masonline.com.ar/api/checkout/pub/orderForms/simulation?sc=1` con `{items:[{id:skuId, quantity:N, seller:'1'}], country:'ARG'}`:
  - `ratesAndBenefitsData.rateAndBenefitsIdentifiers[]` = beneficios aplicados en el carrito (`{id, name, matchedParameters}`). Nombres reales: `"PromoVolumen - LLEVANDO 2 - 2da al 70% - Reg-2-70 - SURTIDO FLASH ANIVERSARIO"`, `"PromoVolumen - LLEVANDO 2 - 2x1 - Reg-2-100 - LEGUMBRES Y GRANOS SECOS CCQ"`, `"2x$2499 ALFAJOR TRIPLE UNIT - ANIVERSARIO NANCY"`, `"PromoVolumen - LLEVANDO 3 - 3x2 - Reg-3-100 - COCA COLA EXCLUSIVO ONLINE"`, `"3X$3999 ARCOR RAMEN - ANIVERSARIO NANCY"`. `interpretarPromoCarrefour` los interpreta todos.
  - **Asociación beneficio → SKU:** por `items[i].priceTags[].name = "discount@price-{idDelBeneficio}#{idAcción}"` (valor negativo en centavos). No por `matchedParameters` (no viene en todos los beneficios). `discount@price-` además excluye los de envío (`discount@shipping-`).
  - **Mecanismo implementado (`AllPromos/core/simulacionChangoMas.js`)**, usado por `scraper-promos-changomas.js`, `completar-changomas-por-ean.js` (solo los SKUs nuevos) y `refrescar-precio-extras-changomas.js`: lotes de 50 skuIds, 1 s entre POST, reintentos con `fetchConReintentoHTTP` (timeout 30 s), **dos pasadas (cantidad 2 y cantidad 3) unidas**. Hace falta la de 3 porque los "LLEVANDO 3" no aparecen simulando 2 (38 SKUs de 2548 el 24/09); y hace falta la de 2 porque simulando 3 VTEX elige la mejor promo para esas 3 unidades y puede tapar un "2da al X%" (visto: SKU 31057, 2da al 50% a cantidad 2, 3x2 a cantidad 3). Cada nombre se guarda como `promosInternas: [{nombre, esBancaria:false}]` → `precioCache.js` → `entradasVtexConTeasers` → `interpretarPromoCarrefour`, sin cambios en el backend. Solo se guardan nombres con formato "por cantidad" (`LLEVANDO N`, `Reg-N-M`, `NxM`/`Nx$`, `Nda al X%`) y que no sean bancarios/MasClub/envío/medios de pago.
  - **Nunca rompe el scraper:** un lote con error (red, 429/5xx tras reintentos, formato distinto) se cuenta y se sigue; 3 lotes fallidos seguidos o pasarse del tope de tiempo (7 min scraper, 3 min refrescador) cortan la pasada. El catálogo se escribe igual (con guardrail y escritura atómica como siempre), sin esas promos, y el log lo dice.
  - **Costo medido 2026-09-24:** ~2,2-2,7 s por lote (1 s de espera incluido), **~270 s las dos pasadas sobre el catálogo base** (51 lotes cada una) → el scraper pasó de ~1,7 min a ~6,2 min (timeout del cron: 15 min). Refrescador de extras: ~+5 s cada 100 SKUs (con ~918 extras, ~+50 s; timeout 5 min).
  - **Cobertura:** 374 de 2550 SKUs (~15%) con promo por cantidad el 24/09 (coincide con la muestra previa de 150 SKUs: 17%). Totales de la app verificados contra la simulación en 12 casos (2x1, 2da al 50/70/80%, 2x$, 3x2, 3x$, promo + descuento directo): iguales, salvo ±$0,58 en 3x2 a cantidad 3 por el redondeo de VTEX al repartir el descuento por unidad.
  - **Detalles de la simulación:** un skuId inexistente no rompe el lote (viene un `messages[]` ORD027); un SKU sin stock viene con `availability: "withoutPriceFulfillment"` y sin priceTags; con cantidad 3 VTEX parte el ítem en varias líneas (2 con promo + 1 sin), por eso se acumula por `id`.
  - **No capturado a propósito — beneficios UNITARIOS de checkout:** `"Promo Banana $1699 23/09"` (el catálogo ya muestra $1.699, no hay diferencia) y `"Ahorrás 35%- Exclusivo online- Carne Picada $10.349"` (SKU 130590: catálogo `Price` $10.349,21, pero el checkout cobra $8.279,37 c/u — diferencia real no capturada). No son promos por cantidad, el motor no los interpreta, y si algún día se interpretaran como % sobre `precioActual` habría doble descuento en casos como la banana. Si se quiere capturar el caso carne picada, la vía correcta sería tomar el precio unitario de la simulación a cantidad 1, no el nombre.
- **`descuentoDirecto.precioSinDescuento`** (desde 2026-09-24, en los 3 scripts de Chango Más, vía `extrasDescuentoDirecto()` de `core/datosPromoVtex.js`, el mismo helper que Carrefour/Día; solo existe cuando hay descuento directo) = `commertialOffer.PriceWithoutDiscount`. Ojo: en Chango Más es casi siempre igual a `Price` (el precio ya con descuento directo), **no** a `ListPrice` (verificado 24/09 sobre 2550 SKUs: 613 difieren de `precioBase`, todos con descuento directo). Las únicas 2 excepciones (difiere de `Price`) son justo los beneficios unitarios de checkout del punto anterior: banana (`PriceWithoutDiscount` 2699 = lista, `Price` 1699) y carne picada (`PriceWithoutDiscount` 13499 = el `price` de la simulación, `ListPrice` 14899, `Price` 10349,21).
...
**Promos exclusivas de Comunidad Coto (investigado en vivo e implementado 2026-09-24).** Comunidad Coto es el programa de socios (gratis) de Coto; en la app es un "descuento propio" que el usuario marca con el nombre canónico exacto **`Comunidad Coto`** (constante `COMUNIDAD_COTO` en `promo-engine.js`).
- **Marca confiable = `discounts[].discountImage` terminado en `comunidad.png`** (`/content/images/cdigi/ofertas/comunidad.png`), helper `esDescuentoComunidadCoto`. **No** usar `sale_type: ["Miembros Comunidad"]`: es del producto entero (convive con un `25%Dto` para todos en el mismo SKU) y puede faltar aunque el descuento sea de Comunidad (EAN 3016661173493: `sale_type` "Exclusivas", discount "1 Pago 30%" con comunidad.png).
- **Formatos** (≈40 SKUs verificados contra `discountPrice`, todos con comunidad.png y "No acumulable con otras promos"; nunca se vio un NxM/"70% 2da"/"X%Dto" con comunidad.png):
  - `"1 Pago X%"` (~215 SKUs, electro/hogar): X% sobre el precio de lista pagando en 1 cuota (las 12 cuotas van aparte en `discounts_payment_methods`). Siempre Comunidad en la muestra (19/19). Ej. $54.999 → `discountPrice` $38.499,30.
  - `"15%"` suelto (3 SKUs, Coca-Cola): 15% sobre lista ($5.150 → $4.377,50).
  - `"+X%"` (56 SKUs, "+5%"/"+10%"): **siempre acompaña a un `N%Dto` para todos del mismo SKU**, y significa X puntos MÁS sobre lista (25%Dto + "+10%" → 35% sobre lista: $3.725 → $2.421,25), no X% sobre el precio ya rebajado.
- **Dónde se guarda:** `scraper-coto-por-ean.js` (`interpretarDescuentos`) marca la promo interna con `comunidad: true` + `precioFinal` (= `discountPrice`). A propósito sin `descuentoPct`/`cantidadMinima` en esa promo: cambiarían la huella de Alertas (`diffCatalogos.js`) y re-avisarían. Catálogos viejos (solo `{nombre, comentarios}`) se reconocen por texto (`esPromoInternaComunidadCoto` → `esPromoCotoCondicionada`).
- **Cómo se aplica:** `promoComunidadCoto` arma un `pct_directo` sobre `precioBase` con `requiereTarjeta: 'Comunidad Coto'`; el % se deriva de `precioFinal` (la verdad) y, sin él (catálogo viejo), del texto (para "+X%" suma el % del `descuentoDirecto`). `precioCache.entradasCoto` genera esa entrada siempre (comparar.js/`promoParaRanking` la ignoran si el usuario no tiene Comunidad marcada, y la muestran como aviso con `totalConTarjeta`); `parsearProductosCoto` (fallback en vivo) solo la genera si `tarjetas` incluye 'Comunidad Coto' (el backend le pasa `TARJETAS_QUE_AFECTAN_PRODUCTO`). "No acumulable": `mejorOpcion` elige una sola entrada por SKU, así que la de Comunidad reemplaza al `X%Dto`, no se suma. Con el catálogo de prod del 24/09: 274 SKUs con promo de Comunidad, en los 274 baja el total para quien la marca. Tests: `AllPromos/core/comunidadCoto.test.js` (fixtures reales en `comunidadCoto.fixture.json`).
- **Quirk de `discountPrice`:** en algunas aguas Sierra de los Padres (EAN 7798065730033) el `discountPrice` no cuadra con el precio dominante (25%Dto sobre $1.950 da $1.398,93, o sea 28,3%; "+5%" da 33,3% en vez de 30%) — probablemente calculado sobre otra sucursal. Se respeta igual `discountPrice` (mismo criterio que el `descuentoDirecto` de siempre).
- **Fix de paso en el fallback en vivo:** un SKU cuyo único descuento era de Comunidad (o con `discountText` null) no generaba ninguna entrada y desaparecía del resultado; ahora cae a precio de lista sin promo.
...
**Formatos de pack ampliados (auditoría 2026-09-24):** la regex única `x N Un` no reconocía "Cerveza Brahma Chopp Lata **473mlx6**" (Vea/Jumbo/Disco) ni "Pack Latas 473 CC **6 Unidades**" (Coto), así que Brahma (EAN 7792798005970) se comparaba contra la lata suelta de Día. `REGEX_MULTIPLICADOR` ahora es una lista de 3 patrones (`x N Un`, medida pegada a `xN`, `N Unidades`). Sobre los catálogos locales pasó de 7 a 9 EAN marcados (se suman Brahma y Andes IPA), cero falsos positivos nuevos. Tests en `AllPromos/core/empaquetado.test.js`.
...
- **Tampoco sirve `discoargentinav700cordoba700` para Disco** (confirmado en vivo 2026-09-24: 200 vacío con ese seller, 10/10 promos con `jumboargentinav700cordoba700` para los mismos SKUs en disco.com.ar). `completar-disco-por-ean.js` y `refrescar-precio-extras-disco.js` lo tenían, así que **todos los extras de Disco quedaban sin promo**; corregido al seller compartido (test que lo fija en `core/cencosud-precio-fijo.test.js`).
...
**Auditoría de promos por súper (2026-09-24) — reglas nuevas del lado de producto** (casos reales verificados contra la simulación de checkout de cada súper; tests en `AllPromos/core/promos-producto-2026-09-24.test.js`, `cencosud-precio-fijo.test.js`, `simulacionChangoMas.test.js`):
- **Descuento directo + teaser no se acumulan cuando el directo es una promo VTEX** (Carrefour y Día). `PriceWithoutDiscount == ListPrice` → el directo es una promo y el súper aplica UNA sola sobre ListPrice: el teaser (2do al X%, NxM, tarjeta propia) se calcula sobre ListPrice. `PriceWithoutDiscount == Price` → precio de tabla, el teaser sí va sobre Price. Los scrapers guardan `descuentoDirecto.precioSinDescuento` y `descuentoDirecto.nombre` (DiscountHighLight); helper `AllPromos/core/datosPromoVtex.js` (`precioBaseTeasers`/`precioBaseTeasersOffer`). Catálogo viejo sin el campo → precio actual (como antes). En Chango Más `PriceWithoutDiscount == Price` casi siempre, así que no cambia nada ahí. Caso: Coca Zero 2,25 L x2 en Carrefour, app $6.637 → $8.850 (lo que cobra el súper).
- **"35% Off Tarjeta Carrefour o Cuenta digital"** vale con Tarjeta Carrefour Crédito O Cuenta Digital Carrefour (`tarjetasDelTeaserPropio`, una entrada por tarjeta) y sobre ListPrice.
- **"Exclusivo online"**: en el nombre del descuento directo (DiscountHighLight de Carrefour) o en el badge `clusterHighlights` "Exclusivo Online" de Día (campo `exclusivoOnline` del SKU) → `esOnline: true`.
- **Formatos de texto nuevos** (`interpretarPromoCarrefour`): "Llevando 2 a $950 c/u" (Día, precio fijo por unidad), "2da al X%" (además de "2do"), `Reg-N-M` alcanza solo, "2x$2499 ALFAJOR…" (precio fijo sin ancla final; montos con formato AR vía `parsearMontoAR`).
- **Cencosud `fixed_price`** ("OFERTA X"): el precio es `value`, no `effectiveDiscount` (que es un % de toda la promo). Ver "API de Vea".
- **Coto "Llevando N" + X%Dto** (campo `takingText`): el % vale solo por grupos de N (`descuentoDirecto.cantidadMinima`, `promoDescuentoCoto`; `calcularCosto` con `pct_directo` y `cantidadMinima > 1` cobra el resto a precio lleno). Antes 38 SKUs (leches, cremas, yogures) aparecían hasta 56% más baratos comprando 1. Textos "15%" suelto, "+5%" y "1 Pago X%" son exclusivos de Comunidad Coto (`discountImage` comunidad.png): ya no se aplican a todos (`esPromoCotoCondicionada`), sino solo a quien marca el descuento propio `Comunidad Coto`, vía `requiereTarjeta` (`promoComunidadCoto`, ver "API de Coto — quirks críticos" → "Promos exclusivas de Comunidad Coto").
- **Fallback en vivo**: si ningún teaser de un SKU se pudo interpretar, igual queda la entrada sin promo (antes el súper desaparecía del producto).
- **No cambiado a propósito**: "Max N unidades" de Carrefour sigue cortando la promo en N (la simulación online la aplica a todas; no confirmado en el local).
...
A diferencia de `promo-engine.js` (promos atadas a un producto), esto cubre el otro tipo de promo: un % de descuento (o cashback) sobre **todo el ticket**, condicionado a día de la semana + banco/tarjeta + a veces canal, con tope de reintegro y a veces monto mínimo — Cencopay, Mi Carrefour (las 3 tarjetas: Mi Carrefour/Cuenta Digital Carrefour/Tarjeta Carrefour Crédito, ver "Corregido 2026-09-04" arriba), MasClub, y bancos/billeteras de terceros (Santander, Mercado Pago, Cuenta DNI, Banco Provincia, MODO, Galicia, Banco Macro, HSBC, BBVA, ICBC, Comafi, Naranja X, Credicoop, Banco Ciudad, Supervielle, Banco Columbia, Banco Patagonia, Banco Nación, Banco Hipotecario, TCI), las promos "<Banco> Modo" (el banco, marcadas `viaModo`) y, desde 2026-09-24, promos sin tarjeta condicionadas a un segmento/programa del cliente (Jubilado, Plan sueldo, Empleado público, Supervielle Identité, MasGO, Comunidad Coto) o para todos (ver "Modelo de tarjetas y requisitos" abajo). Se calcula una vez sobre lo ya asignado a cada súper; hasta 2026-09-24 siempre sobre el subtotal entero, desde entonces solo sobre las filas que cumplen las restricciones de la promo (categorías, exclusiones, "no acumulable" — ver "Promos limitadas a categorías" abajo).
...
**"Galicia" vs. "Galicia Modo".** El feed de Vea trae ambos como entidades distintas: "Galicia Modo" exige pagar escaneando el QR de MODO desde la app Galicia o la app MODO (no alcanza con tener la tarjeta Galicia), mientras que "Galicia" a secas es la promo genérica de tarjeta. Del 2026-08-25 al 2026-09-24 "Galicia Modo" fue un canónico/opción propia; desde el 2026-09-24 (decisión final del usuario, ver "Modelo de tarjetas y requisitos") ya NO es una opción: resuelve a `canonicosPosibles: ['Galicia']` + `viaModo: true` (aplica con solo Galicia marcado), igual que cualquier "<Banco> Modo". Quien lo tenía guardado no pierde nada: `normalizarTarjetasUsuario` (backend) y `migrarTarjetasModo` (app, al hidratar) lo traducen a Galicia + MODO. Ya no hace falta `EXCLUSIONES_ALIAS.Galicia = ['modo']` (se sacó).

**Filtrado conservador:** promos de financiación (cuotas sin interés) se descartan siempre, sin importar la tarjeta — no es un ahorro real de precio. Ojo: `tope: null` en el cálculo significa **sin tope** (el ahorro crece sin límite), así que dejar en `null` un tope dudoso NO es conservador — desde la auditoría 2026-09-24, con varios topes se usa el menor (ver abajo). Solo los ejemplos ilustrativos ("compra de $30.000 → reintegro de $3.000") se ignoran.

**Reglas agregadas en la auditoría de promos del 2026-09-24** (tests con textos reales en `AllPromos/core/bancarias-auditoria-promos.test.js`; antes/después medido sobre los feeds de ese día):
- **Monto mínimo** (`extraerMontoMinimo`): además de "compra mínima"/"a partir de", lee "Mínimo de compra: $75.000", el typo real "Mínmo de compra $30.000" (Chango Más MODO), "compras iguales o superiores a $100.000" / "compras mayor o iguales a $40.000" (MODO en Cencosud), "monto mayor o igual a $35.000" (Día), "superiores a $X". "SIN MÍNIMO DE COMPRA" no matchea. Antes el mínimo quedaba `null` y la promo se aplicaba a tickets chicos (Carrefour MP jueves y Cuenta DNI miércoles: mínimo $15.000).
- **Topes** (`topesEnTexto`/`extraerTope`/`topeDePromo`/`extraerTopeTextoLibre`): (a) varios topes distintos → el **menor** (Coto Comafi 30% online martes: $15.000 general / $25.000 Segmento Único, antes quedaba "sin tope"); (b) tope sin `$` si tiene separador de miles ("TOPE 40.000 POR VIGENCIA", Patagonia en Cencosud) — sin `$` se exige el formato 40.000 para no leer teléfonos (0810-…); montos mal formados ("$10.00", typo real de BBVA) se ignoran; (c) se ignoran topes precedidos (≤40 chars) por "jubilad" y los de "sin tope …"; si no queda ningún tope y el texto dice "sin tope", `null` legítimo (Cuenta DNI 10% miércoles en Carrefour: "¡Sin Tope!" + "Si sos jubilado tope de $5.000" → antes $5.000, y sin la regla caía al ejemplo "reintegro de $3.000"). En Carrefour/Chango Más/Cencosud el texto corto (`sub_title`/`info`) manda sobre el legal, que suele listar topes de otros productos/segmentos (Patagonia Singular $15.000 vs. $10.000 de Clásica).
- **MODO + banco** — aplica con solo el banco (`viaModo`, modelo final del mismo día, tras pasar por `requiereTodas`, por un canónico propio "<Banco> Modo" y por "banco + requisito MODO", todos descartados), ver "Modelo de tarjetas y requisitos" abajo. El bug de fondo sigue corregido: "ICBC Modo", "Banco Credicoop MODO", "Hipotecario_Modo", "Banco_Comafi_MODO" (Chango Más) y en Coto "Pagando con MODO desde la app de Comafi/Supervielle/Ciudad" quedaban como "MODO o el banco" y un usuario con solo MODO recibía el 30% de Comafi.
- **Alias:** `'naranjax'` (Chango Más) para Naranja X; exclusión `'club'` en Banco Nación ("Club La Nación" de Carrefour quedaba como Banco Nación); canónico nuevo **Banco Hipotecario** (`'hipotecario'`, ninguna otra entidad de los feeds lo contiene; 25% martes tope $15.000 en Jumbo/Disco). `'patagonia'` a secas NO se agregó (decisión pendiente por niveles).
- **Canal** (`canalesTicketVTEX`): Chango Más trae UN flag por promo y `express` = presencial Y online (legales "tanto presencial como online") → se le suma `ecommerce`; `market` = solo tiendas. Carrefour: `maxi` (formato mayorista, la app no lo modela) se saca, y una promo que SOLO tenía `maxi` se descarta (Tarjeta Carrefour Crédito 15% lun/mar, MP 10% viernes, Cuenta DNI y Cuenta Digital en Maxi). Promo bancaria solo online (Carrefour Crédito 20% jueves, "NO VÁLIDO EN LAS TIENDAS"): `aplicarPromosBancarias` ahora pone `requiereOnlinePorSuper[key] = true` también cuando la promo elegida `promoBancariaRequiereOnline()` (antes solo por promos de producto online), y `bancario.porSuper[key].soloOnline` — la app ya mostraba "REQUIERE COMPRAR ONLINE" y el badge ONLINE por fila con ese dato. Ojo: no usar `oportunidad.canal`, que en empate online/físico devuelve 'online'.
- **Domingo en Cencosud** (`diasCencosud`): el campo `days` del feed nunca trae 7 (0 de 202 entradas). Se agrega el 7 si el texto enumera días terminando en domingo ("viernes, sábados y domingos" — Cencopay Cuenta 40%) o dice "todos los días" (sin día puntual detrás) con `days` = lunes a sábado. No con "semana de lunes a domingo" (definición de tope semanal) ni "todos los días martes".
- **Local puntual** (`esPromoDeLocalPuntual`, solo Cencosud): se descartan las que dicen "el local / la sucursal (de) Jumbo|Vea|Disco X" o "sucursal de X" en singular (Cencopay 25% martes "EN EL LOCAL DE JUMBO COMODORO"). "En los locales habilitados/adheridos" no matchea.
- **Cuotas leídas como %** (`esFinanciacionVea`): `discountText` es la continuación de `discount` ("25" + "% y 3 cuotas"); si no está vacío y no arranca con "%", el número no es un porcentaje → se descarta (Cencopay "18" + "CSI o 15% y 12CSI" = 18 cuotas, antes 18% de todo el ticket; "6" + "y 12"; Amex "100" + "mil $").
- **Coto "Aplica en los productos sin oferta"** (flag `soloSinOferta`: Banco Ciudad online, MODO martes, Supervielle/Comafi con MODO): `mejorPromoTicket(promos, subtotal, subtotalSinOferta)` usa como base solo lo que no tiene promo de producto (el mínimo se mira contra el total). En `/api/comparar`, el handler marca `_mejores[key].sinOferta` (opción con `total >= totalSinPromo`), `aplicarPromosBancarias` lo pasa como `sinOfertaPorSuper` a `reoptimizarAsignacion`, y `repartirDescuentoBancarioEntreFilas` reparte el descuento solo entre esas filas. Sin ese dato (CLI) se usa el subtotal entero, como antes.
- **Coto fechas puntuales** (`extraerFechasPuntuales`, campo `fechasPuntuales: ['MM-DD']`): "Válido únicamente los lunes 07/09, 14/09 y 28/09" (Credicoop, online y local) se aplicaba todos los lunes; `promosAplicablesHoy` ahora exige la fecha (hora argentina). Si el texto trae "únicamente" + fechas pero no se pueden leer con seguridad (fecha inválida o que no cae en el día de la promo) → se descarta.
- **No se tocó (decisiones de negocio pendientes):** niveles de una misma tarjeta (Naranja X Inicial/Turbo/Épico, Patagonia Clásica/Plus/Singular, Galicia Eminent), topes mensuales/semanales tratados como por ticket. "No acumulables" y promos limitadas a categorías se resolvieron el 2026-09-24/25 (ver "Promos limitadas a categorías"). Segmentos, MasGO, Billeteras Virtuales, "10% todos los medios online", Comunidad Coto y "Modo|BNA" de Día se resolvieron el mismo día (ver el bloque siguiente).

**Modelo de tarjetas y requisitos (decisiones del usuario, 2026-09-24 a la tarde).** Cada promo normalizada lleva `canonicosPosibles` (cualquiera de estas tarjetas; puede venir vacío) y `requisitos` (TODOS tienen que estar marcados). Único criterio de match: `promoAplicaATarjetas(promo, tarjetas)` = requisitos ⊆ tarjetas Y (sin tarjetas en la promo, o alguna de ellas marcada). Lo usan `filtrarPromosBancariasPorTarjetas` (comparar/CLI), la grilla y `misDescuentos.js`. `requiereTodas` desapareció (queda solo una rama de compatibilidad en `promoAplicaATarjetas` para un cache generado antes del cambio, que el cron pisa en ≤2hs). Tests con textos reales: `AllPromos/core/bancarias-tarjetas-segmentos.test.js`.
- **Banco + MODO = el banco, `viaModo: true`** (decisión final del usuario 2026-09-24; ese día pasó por `requiereTodas`, por un canónico propio "<Banco> Modo" — `BANCO_MODO`/`TARJETAS_MODO`/`combinarConModo`, ya no existen — y por "banco + requisito MODO"). En Mis descuentos solo hay "MODO" y cada banco por separado. Una promo "ICBC Modo" queda `{ canonicosPosibles: ['ICBC'], requisitos: [], viaModo: true }` y aplica con SOLO el banco marcado (MODO es la app con la que se paga, no se pide marcarla); con solo "MODO" marcado NO aplica. La opción "MODO" es solo para las promos de MODO a secas (`canonicosPosibles: ['MODO']`). `viaModo` solo cambia la etiqueta: `etiquetaTarjetas` → "ICBC vía MODO" (Día "Modo|BNA" 5% jubilados → `['Banco Nación']` + `requisitos: ['Jubilado']` + `viaModo` → "Banco Nación vía MODO (Jubilado)"). `resolverTarjetasDeNombre` devuelve `{ canonicos, modo }` y `resolverTarjetasPromo` `{ canonicosPosibles, modo }`; `clasificarPromo` (y `fetchCoto`) ponen `viaModo: true` cuando `modo` es true. Se detecta: (a) por el nombre de la entidad: "ICBC Modo", "Hipotecario_Modo", "Banco_Comafi_MODO", "BNA+MODO"; (b) MODO y un banco en la MISMA entrada (Día "Modo|BNA"); (c) Coto: descripción + ícono ("Pagando con MODO desde la app de Comafi/Supervielle/Ciudad", "APP BANCA CREDICOOP con MODO"); (d) texto corto que exige MODO (`textoExigeModo`): Carrefour "Exclusivo pagando con Modo" (BNA 5% jubilados), Cencosud `info` "QR de Modo desde app Galicia y app Modo" (Galicia 20%/30%, Supervielle, Banco Ciudad), "Desde App Hipotecario o App Modo", "APP HSBC ARGENTINA O APP MODO" (HSBC 25% miércoles, que con el canónico propio se perdía porque no existía "HSBC Modo"). Frases con "acumulable"/"no aplica" no cuentan. Un "<banco sin canónico> MODO" ("Yoy_MODO", "Banco San Juan MODO", "Semana_MODO") se descarta (tratarlo como MODO genérico era el bug original: un usuario con solo MODO recibía el 30% de Comafi). Limitación conocida: si una entrada mezcla un banco suelto con un "<Banco> Modo", quedan solo los sueltos. **Compatibilidad:** `normalizarTarjetasUsuario(tarjetas)` (exportada; la usan `promoAplicaATarjetas`, `etiquetaTarjetas` y la grilla) traduce opciones viejas guardadas "<Banco> Modo" → banco + MODO ("Hipotecario Modo" → Banco Hipotecario; "Yoy Modo" → solo MODO); la app hace lo mismo al hidratar (`migrarTarjetasModo` en `carrito.tsx`), así que lo que se vuelve a guardar en `perfil_usuario` ya queda migrado. Un cache de promos generado con el canónico propio ("ICBC Modo" como tarjeta) no matchea a nadie hasta que el cron lo pise (≤2hs).
- **Requisitos = segmentos del cliente + programas/tiendas** (`SEGMENTOS` = Jubilado, Plan sueldo, Empleado público, Supervielle Identité; `PROGRAMAS` = MasGO, Comunidad Coto), opciones propias al final de Mis descuentos. `requisitosDeTexto()` los detecta SOLO en los campos cortos (nombre de entidad, título/sub_title en Carrefour/Chango Más, `info` en Cencosud, título + encabezado corto del legal en Día, `descripcion` + ícono en Coto), nunca en el legal completo. Jubilado = jubilados, pensionados, beneficiarios ANSES, mayores de 60. Dentro de cada frase (corte en ". " que no sea "$15.000", "!", "|", " - "), una mención no cuenta si hay un "tope" antes ("Tope mensual: $10.000 para Cartera General y $30.000 para … Sueldo … y Jubilado", Hipotecario_Modo), si le sigue "tope" ("Si sos jubilado tope de $5.000", Cuenta DNI) o si la frase dice "adicional" ("Y si sos jubilado, tenés 5% adicional"). Tres casos: banco + segmento (Banco Nación vía MODO 5% jubilados en Carrefour y Día, Supervielle "Exclusivo Jubilados"/"MARTES JUBILADOS", Mi Carrefour 10% ANSES/+60, ICBC_Sueldos y Coto ICBC "PLAN SUELDO", Coto Supervielle "SGTO IDENTITÉ", MasClub/Banco Ciudad/MODO "Exclusivo MasGo") → tarjeta Y requisito; solo segmento, sin banco (`clasificarPromo`: entidad "Anses"/"Beneficios Anses"/"Empleados Públicos"/"Medios de Pago" o sin entidad; Coto por `ICONO_REQUISITO_COTO`: comunidad, jubilados y pensionados, beneficios ANSES) → solo el requisito; promo general del banco → no se restringe. Un banco que la app no modela con texto de jubilados se sigue descartando (no se regala a todos los jubilados). Antes varias se descartaban (Anses, Empleados Públicos, Medios de Pago jubilados de Vea/Jumbo/Disco, íconos de Coto) y otras se aplicaban a todos (Supervielle jubilados, Mi Carrefour ANSES, ICBC Sueldos, las MasGO).
- **"Comunidad Coto"**: mismo string que usan las promos de PRODUCTO de Coto como `requiereTarjeta` (promo-engine.js); por eso también está en `TARJETAS_QUE_AFECTAN_PRODUCTO` de `comparar.js`. La promo por ticket (15% miércoles en sucursales) aplica solo a quien la marca.
- **"Billeteras Virtuales"** (Chango Más, 15% vie-dom, "Pagando con QR de todas las billeteras virtuales"; su legal dice QR de Mercado Pago) = alias de Mercado Pago. Ninguna otra entidad de los 5 feeds contiene ese texto. Alias nuevo `'bna'` para Banco Nación (Día "BNA", Chango "BNA+MODO").
- **Para todos**: una promo sin tarjeta ni requisitos (entidad vacía/genérica + "todos los medios de pago"/"cualquier medio de pago" en el texto corto: Carrefour "10% OFF Todos los Medios de Pago - EXCLUSIVO ONLINE", jueves, tope $8.000/semana) aplica a TODOS. Es la única así en los feeds de hoy (las demás "todos los medios" son de segmento). La financiación en cuotas ya se descartaba antes por no tener %. Canal `ecommerce` → `soloOnline`/"REQUIERE COMPRAR ONLINE" como cualquier promo solo online. Para que llegue a usuarios sin tarjetas, `datosBancariosDeHoy` (comparar.js) ya no corta con `tarjetas` vacío: lee el cache igual y el filtro deja pasar solo las promos para todos.
- **Etiqueta** (`etiquetaTarjetas`, "Pagando con X"): la tarjeta + los requisitos entre paréntesis ("Supervielle (Jubilado)"); sin tarjeta, "cualquier medio de pago (Jubilado)" o "cualquier medio de pago".
- **Día — % del título**: `extraerDescuentoPctDia(terms, titulo)` usa un único "N%" del título de la tarjeta SOLO si el legal no trae ningún % legible (no si trae varios distintos): "5% BNA MODO" (el legal solo lo repite en el encabezado "BNA JUBILADOS 5%"). La tarjeta "Anses" de Día sigue descartada: no trae el % en ningún texto.
- **Grilla**: logo = la tarjeta (ICBC vía MODO → logo de ICBC); sin tarjeta, el requisito o "Todos los medios de pago" (iniciales). Prioridad por banco y por celda: primero las que el usuario puede usar; entre las demás, las sin requisito antes que las con requisito (a quien no marcó MasGO no le aparece "MasClub 20%" de MasGO tapando el 15% general); después, mayor %.
- **Mis descuentos**: cada opción lista las promos que la nombran como tarjeta o como requisito; una banco vía MODO va solo bajo su banco (nunca bajo "MODO", así que el % de "MODO" siempre es de MODO a secas). El % destacado (`mejorPromoDeOpcion` en `misDescuentos.js`, también para las no vigentes con día definido) prioriza: 1) las promos que se usan con SOLO esa opción (ICBC vía MODO 20% antes que ICBC + Plan sueldo 30%); 2) las que la tienen como tarjeta aunque pidan otro requisito; 3) cualquiera (Supervielle + Jubilado bajo "Jubilado"). La pantalla no agrupa (lista plana + buscador): segmentos/programas al final.
- `diffCatalogos.js` suma los requisitos a la identidad de una promo (solo si tiene) para no confundir dos promos sin tarjeta del mismo día.

**Promos limitadas a categorías, con exclusiones y "no acumulables" (decisión del usuario, 2026-09-24/25).** Antes toda promo por ticket iba sobre el ticket entero (Cencopay 25% jueves en Jumbo "galletitas, bebidas sin alcohol, perfumería y limpieza" se aplicaba también a la carne). Ahora cada promo normalizada puede traer (solo las claves que aplican):
- `categoriasIncluidas` (vocabulario propio: `almacen`, `galletitas`, `golosinas`, `conservas`, `conservas vegetales`, `cervezas`, `bebidas con alcohol`, `bebidas sin alcohol`, `bebidas`, `lacteos`, `leches`, `quesos`, `fiambres`, `congelados`, `perfumeria`, `limpieza`, `papeles`, `tapas`, `pastas frescas`, `bebe`, `farmacia`, …; lista completa en `VOCABULARIO` de `AllPromos/core/categoriasPromo.js`). Se extrae en `restriccionesDeCategoria(textoCorto, legal)` (`promos-bancarias.js`) de "descuento en [productos seleccionados de (las secciones)] <lista>", "únicamente en los productos <lista>", "en los productos <lista>" y un texto corto que arranca con "En <lista>" (Carrefour Empleado público). La lista se parsea con frases conocidas de la más larga a la más corta (`parsearListaCategorias`; "conservas de verduras, legumbres y frutas" es UNA frase). **Regla de seguridad:** si el texto dice que es solo para ciertos productos ("seleccionados", "únicamente") o la lista empieza con una categoría conocida, y algún pedazo no se puede mapear → la promo se DESCARTA (nunca se aplica al ticket entero). Hoy eso descarta Chango Más ANSES 10% ("únicamente en los productos detallados: … almacén sin tacc, horneables y gelificables…"). Frases como "descuento en tu compra", "en el acto", "en los productos sin oferta" no son listas (primer elemento desconocido).
- `categoriasExcluidas` / `marcasExcluidas` (`exclusionesDeTexto`): zona desde "no incluye / excluidos / se excluye (el rubro) / excepto / no participan" y las oraciones siguientes mientras sean pura lista (Chango Más reparte la lista en 3 oraciones; los saltos de línea no cortan). Términos reconocidos en `TERMINOS_EXCLUSION` (carnicería/carnes/granja, embutidos, huevos, frutas y verduras, leches, leches infantiles, quesos, aceites, harinas, azúcar, bebidas blancas, vinos — no "vinos en brik" —, fernet, yerbas, fideos, yogures, electro, bazar, textil); lo que no se reconoce ("ofertón por bulto", "precios cuidados") se ignora = esas filas siguen contando como antes (acá NO rige la regla de seguridad: solo puede bajar la sobreestimación). A propósito NO se excluyen "cervezas"/"gaseosas"/"bebidas" genéricas: en los feeds solo aparecen como "cervezas y gaseosas de Cervecería y Maltería Quilmes (…)" → marcas. Marcas: diccionario `MARCAS` (Quilmes, Stella, Patagonia, Andes, Corona…, línea Coca-Cola, Pepsi, H2OH, Gatorade…, bodegas Rutini, Chandon, La Rural, Catena…), cada una atada a su categoría (Patagonia solo como cerveza) y matcheada contra el nombre del producto; "no incluye productos Carrefour de alimentos, lácteos, …" = marca propia `carrefour` (cualquier categoría), no esas categorías. "Marcas exclusivas/propias (C&CO, …)" de Cencosud se ignora.
- `noAcumulable` → también `soloSinOferta` (misma base que Coto "Aplica en los productos sin oferta"): "no acumulable con otras promociones/descuentos/ofertas" (Cencopay, Carrefour Empleado público / Mi Carrefour jubilados / Tarjeta Crédito 20% martes, casi todas las de Chango Más, Supervielle/Hipotecario en Cencosud). NO cuenta "…con otras promociones bancarias" (Chango Más Cuenta DNI en el texto corto — su legal sí dice "no acumulable con otras promociones" y ese manda). El texto corto manda sobre el legal ("Promoción acumulable", Carrefour BNA "Acumulable con promociones vigentes"); en el legal, afirmativo + negativo = acumulable. Criterio de cálculo: el % va solo sobre los productos sin promo de producto aplicada (no se evalúa "resignar la promo de producto para usar la bancaria").
- Cencosud: `canales: ['tienda']` si el texto dice "exclusivo (para ventas) presencial(es)" y no habilita web/teléfono (Cencopay 40% vie-dom, Cencopay Cuenta 25% lunes).

**Producto → categorías:** `categoriasDeProducto(ruta)` sobre la `categoria` del catálogo de CADA súper (el mismo EAN tiene otra ruta en cada árbol; `precioCache.categoriasPorEAN(ean)` → `{ vea: 'Almacén > …', coto: '…' }`, y si ese súper no lo tiene, la de otro súper o la del unificado). Reglas sobre la ruta completa con segmentos exactos (no confundir "Quesos y Fiambres", "Galletitas bizcochitos y tostadas", "Aceites, Vinagres y Aderezos > Ketchup"), expandidas a padres (cervezas → bebidas con alcohol → bebidas; galletitas → almacén). Decisiones: "almacén/alimentos secos" = raíz Almacén de cada súper + Desayuno (Carrefour/Día) + raíces planas de Chango Más (incluye galletitas, golosinas, snacks, panificados); lácteos = leche/yogur/queso/crema/manteca/dulce de leche/postres de heladera (no leches vegetales, no tapas); cerveza sin alcohol = cerveza pero no "con alcohol"; bebidas solo bajo una raíz de bebidas; mapeo revisado ruta por ruta contra los catálogos de prod del 2026-09-24 (tests en `core/categoriasPromo.test.js`).

**Cálculo:** `mejorPromoTicket(promos, subtotal, lineas)` con `lineas = [{precio, sinOferta, categorias, nombre}]`: base de cada promo = suma de las filas que cumplen `lineaCumplePromo` (categoría incluida / no excluida / marca no excluida / sin oferta si `soloSinOferta`); tope sobre ese descuento; mínimo contra el ticket entero; devuelve `base`. Sin categoría conocida, la fila NO entra en una promo con `categoriasIncluidas` y SÍ en una con solo exclusiones. `reoptimizarAsignacion` usa la misma base (items con `sinOfertaPorSuper`, `categoriasPorSuper`, `nombre`; `elegirSuperMasBarato` solo cuenta el % si el ítem entra en la promo). Compatibilidad CLI (tercer argumento número o items sin categorías): una promo con `categoriasIncluidas` no aplica; exclusiones no se comprueban. En `/api/comparar` el handler pone `categorias`/`nombreProducto` en cada `_mejores[key]`; `aplicarPromosBancarias` guarda en `resumen._basePromoBancaria[key]` (interno, no viaja) los EAN de la base y `repartirDescuentoBancarioEntreFilas` reparte solo entre esas filas; `bancario.porSuper[key]` suma `categorias` (texto "galletitas, bebidas sin alcohol, perfumería y limpieza" o null) y `noAcumulable`. App: `resultado.tsx` agrega " · solo en <categorías>" a la fila "Pagando con X ahorrás $…"; la grilla (`promosBancariasGrilla.js` → `categorias` por promo) marca el % con "*" y una nota "Solo en algunas categorías de productos". Mis descuentos no muestra % (solo toggles), no se tocó. Ejemplo real (Jumbo jueves, catálogo de prod): galletitas Bagley $3.900 + milanesa $18.999 + Coca Zero $5.900 → Cencopay 25% antes $7.199,75, ahora $2.450 (base $9.800). Tests: `core/bancarias-categorias.test.js`. **Riesgos conocidos:** "productos seleccionados de galletitas…" se toma como toda la sección (sigue sobreestimando dentro del rubro); una lista "descuento en …" de OTRA promo mezclada en el mismo legal de Cencosud restringiría de más; exclusiones no reconocidas siguen contando; productos sin categoría en ningún catálogo (fallback en vivo nuevo) no entran en promos con categorías.
...
**`POST /api/comparar` SÍ conecta este módulo (desde 2026-08-19).** Cuando el body trae `tarjetas` no vacío, `aplicarPromosBancarias()` (`backend/src/routes/comparar.js`) filtra el cache crudo con `filtrarPromosBancariasPorTarjetas()`, lo recorta a promos vigentes **hoy** (`promosAplicablesHoy` — ver por qué en la nota siguiente) y llama `reoptimizarAsignacion()` con los ítems ya resueltos por `calcularResumenFinal`. Si encuentra algo aplicable, pisa `resumen.comprasPorSuper`/`subtotalAsignadoPorSuper`/`requiereOnlinePorSuper`/`totalOptimo` con el resultado y agrega `resumen.bancario` (desglose por super: tarjeta, %, tope, `topeDetectado`, descuento) a la respuesta — ver tipos en `app/src/api.ts` (`RespuestaComparar.resumen.bancario`). Sin `tarjetas`: hasta 2026-09-24 no leía el cache (`bancario: null`); desde entonces lo lee igual para aplicar las promos para todos (Carrefour 10% todos los medios online) — si no hay ninguna vigente, `bancario` sigue en `null`.
...
**Nombre de producto mostrado como EAN crudo en Resultado — corregido 2026-09-04.** `/api/comparar` (`backend/src/routes/comparar.js:385-390`) resuelve el nombre de cada item buscando el EAN en `catalogoUnificado.porEAN()` en el momento de la request — no confía en el nombre que ya mandó el cliente. Si ese lookup devuelve `null` (ventana de carrera con el cron `unificarCatalogo`, que reconstruye `catalogo-unificado.json` periódicamente — puede pasar entre que el carrito agregó el producto y que se pidió la comparación), `item.nombre` llega `null` desde el backend y el frontend caía directo a `item.nombre ?? item.ean`, mostrando el EAN crudo (ej. "7891000368626" para el chocolate blanco Galak que precarga el tour). Fix en `app/app/resultado.tsx`: nueva función `nombreDe(item, itemsCarrito)` que agrega un fallback intermedio — el nombre que el carrito ya tenía guardado para ese EAN (`carrito.tsx` sí lo guarda bien al agregar, nunca lo pierde) — antes de caer al EAN. `enCatalogoLocal` (el flag que ya marca este caso en la respuesta del backend, `comparar.js`) sigue sin consumirse en el frontend; no hizo falta para este fix porque alcanza con el fallback del nombre del carrito.

**"Sin promo vigente ahora" en `/mis-descuentos` — corregido 2026-09-04, decisión de negocio confirmada con el usuario.** `calcularDescuentos()` (`misDescuentos.js`) filtraba las promos de cada tarjeta solo por `ahora >= vigenciaDesde && ahora <= vigenciaHasta` (la ventana de fecha de la campaña), sin mirar `dias`. Tarjetas cuya campaña real (con fecha, ej. MODO/Mercado Pago que se renuevan mes a mes) ya había vencido quedaban con `disponible: false` y **cero información** — el frontend (`descripcionDe()` en `mis-descuentos.tsx`) mostraba el mismo texto fijo "Sin promo vigente ahora" tanto para esas tarjetas (que sí tienen un patrón conocido, ej. "los miércoles") como para tarjetas que directamente nunca tuvieron ninguna promo en el cache — indistinguibles para el usuario, se leía como bug ("¿por qué tantas tarjetas rotas?"). Fix acordado con el usuario: si al menos una promo de esa tarjeta en el cache actual tiene `dias.length > 0` (aunque su `vigenciaDesde/Hasta` ya haya pasado), se muestra igual (`disponible` sigue en `false`, pero viaja `descuentoPct`/`dias`/`tope`/`supers` de la mejor promo con periodicidad conocida). Si no hay ninguna promo con día definido para esa tarjeta, el backend devuelve todo en `null`/vacío y el frontend no renderiza texto (antes de esto era la única forma "vacía" del message, así que no hay import de un caso nuevo, solo se dejó de forzar el texto fijo).
...
**Selección de tarjetas del usuario — una sola fuente de verdad.** No existe una lista de tarjetas separada por pantalla: `carrito.tarjetas` (contexto `ProveedorCarrito`/`useCarrito` en `app/src/carrito.tsx`, campo persistido en AsyncStorage bajo `allpromos:carrito:v1` junto con los ítems del carrito) es leída y escrita tanto por el bloque "Mis descuentos" del carrito (`app/app/(tabs)/carrito.tsx`, solo lectura ahí — tocarlo navega a `/mis-descuentos`) como por los switches de `app/app/mis-descuentos.tsx` (`carrito.setTarjetas(...)`). `TARJETAS_DISPONIBLES` en `carrito.tsx` (31 entradas desde 2026-09-24: sin ningún "<Banco> Modo" — "Galicia Modo" se sacó, se migra a Galicia + MODO — y con los 6 requisitos — Jubilado, Plan sueldo, Empleado público, Supervielle Identité, MasGO, Comunidad Coto —; un test compara sus strings con `TARJETAS_CONOCIDAS`. Lista anterior de 26 — Banco Hipotecario sumado 2026-09-24 —: Mi Carrefour, Cuenta Digital Carrefour, Tarjeta Carrefour Crédito, MasClub, Cencopay, Santander, MODO, Mercado Pago, Cuenta DNI, Banco Provincia, Galicia, Galicia Modo, Banco Macro, HSBC, BBVA, ICBC, Comafi, Naranja X, Credicoop, Banco Ciudad, Supervielle, Banco Columbia, Banco Patagonia, Banco Nación, TCI — los 3 beneficios propios de Carrefour separados 2026-09-04, ver "Corregido 2026-09-04" arriba; los 9 bancos de Cencosud sumados 2026-08-26, ver quirk de Coto/icono de banco más arriba) coincide 1 a 1 con `ALIAS_TARJETAS`/`TARJETAS_CONOCIDAS` de `promos-bancarias.js` — mismos strings canónicos, sin mapeo de por medio. Si se agrega un banco nuevo a `ALIAS_TARJETAS`, hay que sumarlo también acá a mano (no se deriva automáticamente); `mis-descuentos.tsx`, en cambio, sí deriva de `TARJETAS_CONOCIDAS` sin necesidad de tocarlo. Ese array llega tal cual al backend en cada `POST /api/comparar` como `tarjetas`; el backend nunca lee `mis-tarjetas.json` (eso es solo del CLI, ver `leerMisTarjetas()` más abajo).
...
Los 2 que faltan (`Galicia Modo` — opción que ya no existe desde 2026-09-24 — y `Tarjeta Carrefour Crédito`/`Cuenta Digital Carrefour` en
...
Cómo funciona: `backend/src/cron/diffCatalogos.js` compara, en memoria, el catálogo (o `promos-bancarias.json`) de ANTES de que el scraper lo pise contra el de DESPUÉS, y guarda un resumen en la tabla Supabase `scraper_diffs` (migración `0014_scraper_diffs.sql`, **corrida en el proyecto real** — confirmado en vivo el 2026-09-11 consultando la tabla directo con `supabase-js`: 405 filas). Se llama desde el mismo `refrescar()` de `refrescarCatalogos.js`, una fila por super y por corrida:
...
**Huecos de `scraper_diffs` sin catálogo truncado (distinto a los glitches de arriba) — investigado 2026-09-18:** además de los glitches de catálogo, hay corridas donde el diff directamente no se insertó en Supabase aunque el scraper funcionó bien. Investigado con el CLI de Supabase (`npx supabase`, logueado con `SUPABASE_ACCESS_TOKEN`, proyecto ya linkeado) contra `edge_logs`/`postgres_logs`:
- **Explicado — Coto perdió ~15 corridas seguidas entre el 08/09 y el 10/09**, coincide exacto con `❌ Coto (1500s) Command failed` en `cron.log` (timeout del scraper viejo), resuelto por el commit `ec2e3ff` del 10/09.
- **Explicado — 5-6 fallas puntuales aisladas** (Chango Más, Disco, Vea, Día en distintas corridas): el log muestra `⚠️ No se pudo registrar el diff de X: TypeError: fetch failed` / `Gateway Timeout`. La VM es un `e2-micro` (958MB RAM, con swap en uso incluso en reposo) y el proyecto de Supabase está en **plan free** (compute compartido, logs con 1 día de retención) — ambos son candidatos plausibles a esos hipos de red puntuales; no se pudo aislar cuál pesó más en cada caso porque los logs de Supabase de esas fechas ya se purgaron (>1 día) y la Logs Explorer API devolvió `"Backend error"` en consultas de rango amplio al momento de investigar. No es alarmante: solo se pierde una fila de monitoreo, nunca el catálogo real.
- **Falsa alarma descartada — NO es un hueco real:** durante esta misma investigación pareció que la corrida del 18/09 00:00 UTC tenía 5 de 7 supers sin diff. Era un artefacto de haber leído la tabla completa mientras el cron todavía estaba corriendo en la VM (tarda ~18 min en insertar las 14 filas de una corrida) — las 7 filas de productos ya estaban completas al re-consultar después. Ojo con esto si se vuelve a hacer un volcado completo de la tabla mientras el cron puede estar en curso.

**Fix aplicado (2026-09-18):** `registrarDiff()` en `diffCatalogos.js` ahora reintenta hasta 2 veces (3s de espera) antes de rendirse y loguear el warning — reduce el hueco causado por los hipos transitorios de red/gateway de arriba, sin agregar riesgo (nunca hace `throw`, sigue sin frenar el cron si las 3 fallan).

## VM (e2-micro): limpieza de RAM/CPU y migración de crons a GitHub Actions (2026-09-18)

**Hallazgo grande, no buscado a propósito:** investigando por qué la VM andaba justa de RAM, se encontró con `ps aux --sort=-rss` que el Google Cloud Ops Agent (`otelopscol` + `fluent-bit`, ~90MB) más `multipathd` (~28MB, inútil en un disco único), `google_osconfig_agent` (~23MB), `snapd` (~13MB) y `packagekitd` (~6MB) sumaban ~160MB de RAM sin aportar nada al producto (nadie miraba Cloud Monitoring, el uptime se controla con UptimeRobot). Se desactivaron los 5 (`systemctl stop` + `disable`, y `mask` para packagekit) — RAM usada bajó de 366MB a ~250MB.

**El hallazgo más importante fue de CPU, no de RAM:** comparando `compute.googleapis.com/instance/cpu/utilization` (métrica del hypervisor, no depende de agentes de guest) del mismo horario un día antes vs. después de sacar el Ops Agent, en una ventana sin cron corriendo: **23-33% antes → 1-14% (bajando a ~0%) después**, confirmado también con `vmstat`/`top` en vivo dentro de la VM. El Ops Agent no solo pesaba en RAM — hacía trabajo de CPU constante recolectando/exportando telemetría, y eso explicaba casi toda la "CPU de fondo" elevada que tenía la VM 24/7, no los crons.

**Lo que sí sigue siendo un problema real: picos de 85-95% de CPU durante la ventana del cron** (~17% del tiempo total, cada 2hs). Mirando `cron.log` minuto a minuto contra los timestamps exactos de los picos (ej. `2026-09-17T06:14:57Z`), coinciden con los scripts **"extras"** (completadores de catálogo cruzado por EAN, ej. Jumbo/Disco) corriendo entre el minuto 13 y 15 de la corrida — no con los scrapers principales en sí (que son más I/O-bound, con sleeps entre páginas).

**Plan de migración (en curso):** mover a un workflow de GitHub Actions (repo público → minutos gratis sin límite) todo el pipeline que no necesita credenciales de mail/push:
1. Bajar por SCP los `catalogo-*.json` actuales de la VM (sirve de "antes" para el guardrail de `guardrailCatalogo.js` y para el diff, sin tocar ese código).
2. Correr los 7 scrapers + sus "extras".
3. Calcular el diff y registrarlo en `scraper_diffs` (usa 2 secrets nuevos en GitHub: `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`, copia de los que ya están en `backend/.env` — decisión consciente, no se replican credenciales de mail/push).
4. Unificar catálogo + descargar imágenes + refrescar promos bancarias (sin secrets).
5. Subir todo de vuelta a la VM por SCP, reusando los secrets que YA existen en el repo (`VM_HOST`/`VM_USER`/`VM_SSH_KEY`, los mismos de `.github/workflows/deploy.yml`) — cero secrets nuevos para esta parte.
6. Disparar por SSH los avisos de productos seguidos (se quedan corriendo en la VM con sus credenciales de siempre, nunca salen de ahí).
7. Sacar `refrescarCatalogos.js` del crontab de la VM una vez validado a mano.

Con la limpieza de RAM/CPU de arriba, esta migración deja de ser urgente por supervivencia de la VM y pasa a ser una mejora de margen — igual vale la pena por los picos de 85-95% que siguen ahí.

**Migración ejecutada y validada en vivo (2026-09-18):** los 3 archivos nuevos (`refrescarCatalogosGHA.js`, `postProcesarCatalogos.js`, `.github/workflows/scrapers.yml` + los scripts de `.github/scripts/`) se probaron con corridas reales de punta a punta — scrapers en GitHub Actions, catálogos subidos por SSH/tar, post-proceso en la VM, `/api/comparar` respondiendo con los datos frescos, y las 14 filas de `scraper_diffs` (7 productos + 7 promos_bancarias) bajo el mismo `corrida_en`. Se sacó `refrescarCatalogos.js` del crontab de la VM (backup del crontab viejo en `/tmp/crontab-backup.txt` en la VM, por si hace falta revertir).

**Problema encontrado y corregido: el `schedule` nativo de GitHub Actions NO es confiable.** De ~5 disparos esperados cada 2hs en una ventana de 9 horas, **solo 1 llegó a correr** (con 40 min de atraso) — el resto se descartó en silencio. Esto está documentado por GitHub mismo: el evento `schedule` puede demorarse o directamente no dispararse en momentos de carga alta de su infraestructura, sin garantía real de puntualidad — más común todavía en repos públicos/gratuitos como este. Confirmado con `gh run list --json event` filtrando por `event=="schedule"`.

**Fix:** en vez de depender de `schedule`, el cron de la VM (que sí demostramos que es puntual, ver más arriba) dispara el workflow él mismo vía la API de GitHub (`POST /repos/.../actions/workflows/scrapers.yml/dispatches`), con un Personal Access Token fine-grained (scope "Actions: Read and write", solo este repo) guardado como `GH_TRIGGER_TOKEN` en `backend/.env` de la VM — **nunca en el repo ni en GitHub Secrets**, porque es al revés de los otros secrets (la VM llama a GitHub, no GitHub a la VM). El script (`backend/scripts/disparar-scrapers.sh`) es una sola llamada HTTP liviana — no reintroduce el problema de CPU/RAM que motivó sacar los scrapers de la VM. El `schedule` del workflow se dejó en el YAML como respaldo (si alguna vez sí dispara, no hace daño — el guardrail/diff toleran una corrida extra), pero el mecanismo real y confiable es el cron de la VM.
...
**Fix aplicado (2026-09-22):** la lógica de paginación en sí también se tocó, porque el corte se siguió repitiendo (esta vez en Día: 1049 SKUs vs. 2550 esperados, bloqueado por el guardrail). Causa real: los 6 scrapers VTEX (`scraper-promos-{vea,carrefour,changomas,dia,jumbo,disco}.js`) daban por terminado el catálogo apenas una página devolvía menos de `PAGE_SIZE` (50) ítems, sin confirmar — y VTEX a veces devuelve una página corta a mitad de camino (índice con lag), no por haber llegado al final real. Ahora, cuando una página viene corta, el loop la reintenta hasta 2 veces más (3s de espera entre intentos) antes de aceptarla como el fin del catálogo; si en algún reintento vuelve a traer `PAGE_SIZE` o más, sigue paginando normal. Esto es independiente del retry por 429/5xx que ya tenían Carrefour y Chango Más (ese es por error HTTP; este es por una respuesta 200 con menos ítems de los esperados). Coto no tiene este problema (scrapea por EAN, no pagina).

**Ajuste (2026-09-23):** el fix de arriba (2 reintentos de 3s) no alcanzó para un incidente más severo: en la corrida de las 14:00 UTC, **Vea, Jumbo y Disco** (los 3 comparten la misma cuenta VTEX de Cencosud, ver "Jumbo/Disco = Vea, misma cuenta" más abajo) cortaron casi en el mismo punto (~848-899 SKUs de ~2550, un 33-35%) EN LA MISMA CORRIDA, y Vea volvió a fallar en la corrida siguiente (14:37 UTC) — recién se recuperó solo en la corrida de las 16:00. A diferencia del caso de Día (una sola página corta, aislada), acá el corte se sostuvo el tiempo suficiente como para que 6s totales de reintento (2×3s) no lo agarraran: parece un throttle real y más largo de esa cuenta VTEX compartida, no un glitch de una página puntual. **Evaluado y descartado usar cuentas VTEX separadas por sitio** — no es algo que se pueda elegir desde acá: Vea/Jumbo/Disco comparten una única cuenta VTEX de Cencosud por decisión de infraestructura de ellos (mismo `skuId`/EAN/master data en los 3 sitios, confirmado en vivo), no algo que el scraper controle pidiendo distinto. Se subió el retry de confirmación de página corta de 2×3s a **3 intentos de 30s** en los 6 scrapers VTEX (no solo los 3 de la cuenta compartida, por consistencia — Carrefour/Chango Más/Día son cuentas independientes pero el mismo tipo de corte ya les pasó una vez a Día). Sigue dentro del timeout de cada scraper (15-25 min) y del workflow (45 min) con margen de sobra.

**Fix aplicado (2026-09-22, mismo día):** barrido de retry en TODOS los puntos de red de `AllPromos/` que todavía podían morir por un error de conexión (timeout, reset, DNS) sin reintentar — disparado por un fallo real de `refrescar-precio-extras-carrefour.js` (conexión cortada contra Carrefour, sin status HTTP de por medio, así que el retry existente por 429/5xx no lo atrapaba). Alcance:
- `core/batchPorSkuId.js` y los 6 `completar-*-por-ean.js`: su `getJSON()` (basado en `https.get`) solo reintentaba sobre 429/5xx; ahora el handler `.on('error', ...)` también reintenta con el mismo backoff de 10s antes de rendirse.
- `scraper-promos-{vea,jumbo,disco}.js`: su `getCatalogPage()` no tenía NINGÚN retry (ni por status ni por red) — a diferencia de Carrefour/Chango Más/Día, que sí. Ahora los 6 tienen el mismo patrón: retry por error de conexión + por 429/5xx (antes Carrefour y Día solo retryeaban 429, ahora también 5xx, igual que Chango Más).
- `scraper-coto-por-ean.js` (`buscarPorEAN`, `getOfertasPage`): el `fetch()` no estaba en un `try/catch`, así que un error de red se colaba sin pasar por el retry de status ya existente y tiraba abajo el `pool()` completo (miles de EAN en un solo request roto). Ahora sí está cubierto.
- `refrescar-precio-extras-{vea,jumbo,disco}.js` y `completar-vea-por-ean.js` (`postJSON`, usado para `_v/search-promotions`): antes cualquier error (conexión o HTTP) resolvía `null` en silencio, sin reintentar ni loguear — el lote de promos quedaba vacío esa corrida sin que nada lo mostrara. Ahora reintenta igual que los `getJSON()` (10s, hasta agotar), y solo cae a `null` si se agotan los intentos.
- `promos-bancarias.js`: las 4 fuentes (`fetchCencosud`/Vea-Jumbo-Disco, `fetchGraphQL`/Carrefour-Chango Más, `fetchDia`, `fetchCoto`) ya envolvían el fetch en un `try/catch` que devuelve `{error: 'fetch_failed'}`, pero sin reintentar nunca un corte de conexión. Se agregó `fetchConReintento()` (2 intentos, 3s de espera) usado en las 4 — no toca el manejo de errores HTTP/lógicos que ya tenían (ej. `hash_roto`), solo el caso de que la conexión ni siquiera llegue a responder.

Lo que NO se tocó a propósito: los errores de status HTTP "reales" (4xx que no sea 429, JSON malformado, hash de GraphQL roto) siguen sin reintentar — son errores que un retry no arregla, hay que corregir código o credenciales.
...
## Aviso de "promo nueva" en productos seguidos — pantalla "Alertas" (backend hecho, 2026-09-10 — UI en construcción)

Diseño cerrado en Claude Design (proyecto "SuperApp", `AllPromos v2.dc.html`, turno 20, pantallas 20a-20g): nueva pestaña "Alertas" en el lugar de "Ahorros" en la nav bar (Buscar · Carrito · Descuentos · Alertas · Ajustes); "Ahorros" se mudó a ser una fila dentro de Ajustes.
...
**A propósito NO es un resumen diario.** Se planteó esa alternativa (un mail agrupado una vez por día, a las 9hs) durante el diseño y se descartó el mismo 2026-09-10: el usuario prefiere enterarse en la misma corrida del cron (cada 1-2hs) en la que la promo se prendió, no esperar hasta la mañana siguiente. El "agrupado" que sí queda es horizontal (un solo mail/push por usuario por corrida, con todos los productos que se prendieron en esa corrida), no temporal.
...
**Detección — `estadoPromoPorEan()` en `backend/src/cron/diffCatalogos.js`:** a diferencia de `diffProductos()` (que sí es un diff contra la corrida anterior, para `scraper_diffs`), esto expone el estado ACTUAL de qué EAN tienen promo de producto en esta corrida puntual, con una `huella` (mismo formato que `huellaPromoSku`) que identifica esa promo puntual. Deliberadamente **no** cuenta `promosBancarias` como "promo nueva" — es una promo de tarjeta/banco, no del producto en sí, y ya tiene su propio mecanismo de aviso (ver `aviso-promo-sin-aplicar-solo-si-gana` en la memoria del proyecto).

**Bug real encontrado y corregido antes de correr en producción (2026-09-10, reportado por el usuario):** la primera versión de esto (`productosConPromoNueva()`, ya no existe) era un diff GLOBAL contra la corrida anterior — solo disparaba cuando una promo pasaba de no-existir a existir en esa corrida puntual. Eso se perdía el caso de alguien que empieza a seguir un producto que YA tenía promo activa desde antes: esa transición global ya había pasado, no iba a volver a ocurrir, y el usuario se quedaba sin avisar para siempre. El fix cambia dónde vive la idempotencia: en vez de un estado global ("¿esta promo es nueva?"), se compara por CADA fila de `producto_seguido` contra su propia `huella_promo_avisada` (columna nueva en la migración `0016`) — "¿ya le avisé a ESTE usuario de ESTA promo puntual?". Como una fila recién creada arranca con `huella_promo_avisada = null`, la primera corrida después de seguir un producto con promo activa ya dispara el aviso, sea o no "nueva" a nivel global.

**`avisoProductosSeguidos.js`:** lee TODAS las filas de `producto_seguido` (no solo las de un EAN puntual) con `clienteSupabaseAdmin` (service role), y por cada una compara `estadoActualPromoPorEan.get(fila.ean)?.huella` contra `fila.huella_promo_avisada`: si difieren y hay promo activa, se agrega a la lista de esa `usuario_id` para avisar y se actualiza la huella; si no hay promo activa pero la fila tenía una huella guardada, se resetea a `null` (para que la próxima vez que se prenda, sea la misma promo u otra, vuelva a avisar). El interruptor único (`perfil_usuario.alertas_activas`) se chequea ACÁ, no antes: si está apagado no se manda mail/push y tampoco se actualiza la huella, para que si el usuario lo reactiva más adelante todavía le llegue la promo que siga vigente en ese momento. Después de decidir a quién avisar, agrupa por `usuario_id` y arma un mail (mismo `armarMailBase` de `plantillaMail.js`) y un push (`clientePush.js`, `obtenerSuscripcionesPorUsuario`) listando los productos y en qué super(s) están.

**`refrescarCatalogos.js`** arma `estadoActualPromoPorEan` leyendo del disco el catálogo de los 7 supers DESPUÉS de que todos los scrapers terminaron (no en memoria del loop de cada scraper) — así, si un scraper puntual falla en una corrida, igual se usa el catálogo vigente (el de la corrida anterior) en vez de saltearse ese super por completo. Un mismo EAN con promo en más de un super se resuelve por orden de `SCRAPERS` (primero que aparece, gana) — misma simplificación que ya existía antes de este fix, no es nueva.

**Tabla `producto_seguido`** (migración `0016_producto_seguido.sql`): `usuario_id` + `ean` + `nombre` (desnormalizado, para que la pantalla "Alertas" no necesite una consulta extra al catálogo) + `huella_promo_avisada` (ver el fix de arriba). Sigue el mismo patrón de `push_suscripcion` (`0012_push_suscripciones.sql`): la app escribe/borra su propia fila **directo contra Supabase vía RLS**, sin pasar por un endpoint Express — así es como ya funciona `carrito_guardado`, `ahorro_registro` y `perfil_usuario` desde la app. A diferencia de `push_suscripcion` (que no tiene policy de `select` porque es invisible para el usuario), acá sí hay policy de `select`: el usuario necesita ver y editar la lista de lo que sigue. Un trigger (`tope_producto_seguido`, migración `0017`) impide superar 20 filas por usuario — refuerza en la base lo que la UI ya muestra como contador "N de 20", porque la escritura es directa desde la app y nada del lado del backend la valida.

**`perfil_usuario`: la app solo puede escribir 6 columnas (migración `0024`, 2026-09-24).** RLS limita la *fila* (`auth.uid() = id`) pero no las *columnas*. Hasta la 0024, `authenticated` tenía UPDATE sobre la tabla entera, así que cualquier usuario podía darse `plan='premium'`/`premium_manual=true` desde la consola del navegador (el Auth Hook lo copiaba al JWT). Ahora `authenticated` solo tiene `UPDATE (carrito_items, carrito_tarjetas, supers_activos, tope_supers, tour_visto, alertas_activas)` y `SELECT`; `anon` no tiene nada. Todo lo de plan/cobro/trial/nombre lo escribe únicamente el backend con service_role. **Si una feature nueva necesita que la app escriba otra columna de `perfil_usuario` desde el cliente, hay que agregarla con `grant update (col) ... to authenticated` en una migración nueva**, si no el update falla con 403 / `42501`. Verificado en prod con la cuenta de prueba: PATCH de `plan`/`premium_manual`/`trial_termina_en`/`nombre` → 403, `tour_visto` → 204. Además `procesarSuscripcion` ignora una suscripción de MP cuyo `external_reference` no coincide con el usuario de la fila.

**Migración `0017_alertas_resumen_y_categorias.sql`:**
- `perfil_usuario.alertas_activas` (boolean, default `true`): el único interruptor "Recibir notificaciones" del diseño (push + email juntos, sin split por canal) — `avisoProductosSeguidos.js` lo chequea antes de mandar cualquiera de los dos canales.
- `categoria_seguida` (`usuario_id` + `categoria`, mismo patrón RLS que `producto_seguido`): la pantalla "Alertas" tiene un modo "Categorías" donde se puede elegir y guardar, con un banner "EN CAMINO" — esta tabla solo persiste la elección, **no dispara ningún aviso todavía**. Activarlo es la fase futura de categoría mencionada en el punto de abajo.

**App ya construida (2026-09-10):** nav bar (`app/app/(tabs)/_layout.tsx`, tab "Alertas" con ícono de campana), pantalla `alertas.tsx` (modo Productos/Categorías + interruptor único, reusa `FilaToggleAnimada` en vez de un switch nuevo), `app/app/seguir-producto.tsx` (reusa `buscarProductos` de `api.ts`, selección múltiple + "Guardar" en batch), `src/componentes/DejarDeSeguirHoja.tsx` (hoja de confirmación 20f/20g, mismo patrón que `GuardarCarritoHoja.tsx`), y la reestructuración de Ajustes en 3 grupos (TU ACTIVIDAD/PREFERENCIAS/CUENTA) con "Mis ahorros" mudado a `app/app/mis-ahorros.tsx` y las pantallas nuevas `datos-personales.tsx`/`ayuda.tsx`. Todo esto habla directo con Supabase vía `src/alertas.ts` (mismo patrón RLS que `push.ts`/`plan.ts`), sin endpoints Express nuevos.

**Migraciones `0016`/`0017` ya corridas en el proyecto real (2026-09-10, `supabase db push --linked`)** — confirmado con `--dry-run` que la base remota quedó al día.

**Pendiente para que esto llegue a producción:**
1. Confirmar que `BREVO_*`/`VAPID_*` estén cargadas en el `.env` de la VM (ya deberían estarlo desde los tipos 2-7, ver `mails_y_notificaciones.md`).
2. Probar en navegador real el flujo completo (seguir un producto, ver la fila amarilla en Alertas, dejar de seguir con la hoja de confirmación, tocar el interruptor único) — no se hizo todavía en esta sesión.
3. Fase futura, no bloqueante: activar el aviso por categoría — requiere primero resolver el mapeo entre los árboles de categoría de los 7 supers. `categoria_seguida` ya existe y ya guarda la elección del usuario (pantalla Alertas, modo Categorías); falta la lógica de detección/aviso del lado del cron.
...
**Actualización (2026-08-20), tras el filtro de SKUs fantasma de Coto (ver quirk arriba):** con `catalogo-coto.json` en 2.889 SKUs reales (no 5.000) y `catalogo-vea.json` regenerado sin la cookie rota, `catalogo-unificado.json` bajó de 11.586 a **9.706 productos únicos por EAN** (`node backend/src/cron/unificarCatalogo.js`) — bien lejos todavía del límite de RAM que motivó el cap original, así que no cambia ninguna decisión, solo el número.
  ...
  **Bug preexistente encontrado al verificar el deploy (2026-09-10): el paso de Coto venía fallando por timeout en la VM desde el 9/9, sin relación con el cambio de arriba.** `refrescarCatalogos.js` corre cada scraper con un `timeoutMs` propio (Coto: 25 min); revisando `backend/logs/cron.log` en la VM real, el paso de Coto tiraba `❌ Coto (1500s)` (= timeout exacto de 25 min) en la mayoría de los ciclos desde el 9/9 06:00, con algún ciclo suelto completando cerca del límite (ej. 31 min). Causa real, confirmada en vivo: `buscarPorEAN` hacía un pedido a la vez, secuencial (`await` + `sleep(85ms)`) — en una red local (Argentina, como se midió originalmente el 25-08) la latencia por pedido es casi nula y esto da ~9 min para ~6.500 EAN, pero la VM está en `us-east1` (EE.UU.) y cada pedido a la API de Coto (Argentina) tarda ~200-270ms de ida y vuelta (medido en vivo desde la VM con `curl`) — sumado sobre ~7.500 EAN, eso solo ya da ~35-40 min, antes de sumar la fase de ofertas nueva.

  **Fix: paralelizar en vez de subir el timeout o el pace.** `scraper-coto-por-ean.js` ahora usa un pool simple de `CONCURRENCY_EAN = 8` pedidos en simultáneo (función `pool()`, sin dependencias) en vez del loop secuencial — 8 en paralelo con la latencia real de la VM da un rendimiento agregado bien por debajo del rate limit documentado (~67 req/s), con margen de sobra. **Medido en la VM real (10-09):** la corrida completa (7.757 EAN + fase de ofertas) pasó de fallar por timeout a completarse en **3m54s**. Subir el `timeoutMs` en vez de esto habría escondido el síntoma sin arreglar por qué tardaba tanto, y no hubiera dejado margen para que el catálogo siga creciendo.
...
**Segunda tanda (misma fecha)**
- **Clasificación de teasers bancarios:** un solo criterio, `esTeaserBancario()` en `promo-engine.js` (`/tarjeta|cuenta digital|banco|\bbin\b/i`), usado por scrapers, completadores, refrescadores, `core/fetchers.js` y `precioCache.js`. Antes 10 archivos usaban `includes('bin')` y "Com**bin**able" lo contiene: ~561 SKUs de Carrefour perdían su 2do al X%/NxM en el camino cacheado. `precioCache.js` junta `promosInternas`+`promosBancarias` al leer y las reclasifica, así que aplica también a catálogos viejos. Los teasers "Mi Crf" (Mi Carrefour) se aplican a todos, igual que en el fallback en vivo (pendiente decidir si condicionarlos a la tarjeta 'Mi Carrefour').
- **Reembolsos/contracargos:** `procesarPagoUnico` enruta por estado; un pago del permanente `refunded`/`charged_back`/`cancelled` (con `date_approved` y que coincida con `pagado_en`) baja a gratis. Una cuota de suscripción reembolsada no toca el plan (hay que cancelar la suscripción). Una disputa abierta (`in_mediation`) no quita el plan.
- **Webhook de MP enrutado por tópico** (`resolverEventoWebhook`/`aplicarEvento` en `webhookMercadoPago.js`): payment, subscription_preapproval (o legado sin tópico), subscription_authorized_payment (vía `Invoice.get` → `preapproval_id`), chargebacks (vía `Chargeback.get` → `payment_id`). Tópicos desconocidos y recursos 404 → 200 + log (MP no reintenta); errores reales → 500.
- **Baja de mails con un clic:** `perfil_usuario.mails_no_transaccionales` (0027, solo la escribe el backend). `GET /api/mails/baja?u=&t=` muestra confirmación (no da de baja: los escáneres de links hacen GET); `POST` da de baja (botón o RFC 8058 `List-Unsubscribe=One-Click`). Token = HMAC-SHA256(`BAJA_MAILS_SECRET`, usuarioId), sin expiración; el secreto está en el `.env` de la VM. Corta solo mails de resúmenes, inactividad y Alertas (los push siguen con `alertas_activas`). Para volver a suscribirse, pedirlo a contacto@ (no hay UI).
- **Permisos Supabase (0028):** `anon` no tiene ningún privilegio de tabla ni secuencia en `public` (también para tablas futuras vía default privileges). `manejar_usuario_nuevo()` con `search_path=public`.
- **CI con huella SSH fija:** secrets `VM_HOST_KEY` (clave ed25519 de la VM, la usan `bajar/subir-catalogos.sh` con `StrictHostKeyChecking=yes`) y `VM_HOST_FINGERPRINT` (SHA256 de la clave **ECDSA**, que es la que negocia `appleboy/ssh-action` en `deploy.yml`). Si se recrea la VM hay que actualizar ambos con `/etc/ssh/ssh_host_{ed25519,ecdsa}_key.pub`.
- **VM:** se borró la regla de firewall `default-allow-rdp` (Linux, nada escuchaba en 3389); `.env` y sus backups con permisos 600.
