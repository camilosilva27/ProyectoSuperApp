...
A diferencia de `promo-engine.js` (promos atadas a un producto), esto cubre el otro tipo de promo: un % de descuento (o cashback) sobre **todo el ticket**, condicionado a día de la semana + banco/tarjeta + a veces canal, con tope de reintegro y a veces monto mínimo — Cencopay, Mi Carrefour (las 3 tarjetas: Mi Carrefour/Cuenta Digital Carrefour/Tarjeta Carrefour Crédito, ver "Corregido 2026-09-04" arriba), MasClub, y bancos/billeteras de terceros (Santander, Mercado Pago, Cuenta DNI, Banco Provincia, MODO, Galicia, Galicia Modo, Banco Macro, HSBC, BBVA, ICBC, Comafi, Naranja X, Credicoop, Banco Ciudad, Supervielle, Banco Columbia, Banco Patagonia, Banco Nación, Banco Hipotecario, TCI), las combinaciones "<Banco> Modo" y, desde 2026-09-24, promos sin tarjeta condicionadas a un segmento/programa del cliente (Jubilado, Plan sueldo, Empleado público, Supervielle Identité, MasGO, Comunidad Coto) o para todos (ver "Modelo de tarjetas y requisitos" abajo). No depende de qué productos hay en el carrito: se calcula una vez sobre el subtotal ya armado por super.
...
**"Galicia" vs. "Galicia Modo" — dos canónicos separados a propósito.** El feed de Vea trae ambos como entidades distintas: "Galicia Modo" exige pagar escaneando el QR de MODO desde la app Galicia o la app MODO (no alcanza con tener la tarjeta Galicia), mientras que "Galicia" a secas es la promo genérica de tarjeta. Decisión del usuario (2026-08-25): tratarlas como dos tarjetas separadas en las 3 listas (`ALIAS_TARJETAS`, `TARJETAS_DISPONIBLES` del carrito, `TARJETAS_CONOCIDAS`/Mis descuentos), igual que ya se hace con MODO vs. Mercado Pago. Desde 2026-09-24 el mismo criterio se generalizó a todos los bancos ("<Banco> Modo", ver "Modelo de tarjetas y requisitos"): "Galicia Modo" ya no tiene alias por substring ni hace falta `EXCLUSIONES_ALIAS.Galicia = ['modo']` (se sacó) — el nombre resuelve a Galicia + MODO y `combinarConModo()` lo convierte en el canónico "Galicia Modo" (mismo string de siempre, los usuarios que lo tenían guardado no pierden nada).
...
- **MODO + banco** — ~~exige las dos (`requiereTodas`)~~: reemplazado el mismo día por el canónico propio "<Banco> Modo", ver "Modelo de tarjetas y requisitos" abajo. El bug de fondo sigue corregido: "ICBC Modo", "Banco Credicoop MODO", "Hipotecario_Modo", "Banco_Comafi_MODO" (Chango Más) y en Coto "Pagando con MODO desde la app de Comafi/Supervielle/Ciudad" quedaban como "MODO o el banco" y un usuario con solo MODO recibía el 30% de Comafi.
...
- **"<Banco> Modo" es su propio canónico** (`BANCO_MODO`/`TARJETAS_MODO`): Galicia Modo (nombre de siempre, no se renombró), Santander Modo, Banco Macro Modo, BBVA Modo, ICBC Modo, Comafi Modo, Credicoop Modo, Banco Ciudad Modo, Supervielle Modo, Banco Nación Modo, Hipotecario Modo, Yoy Modo (Yoy a secas no tiene canónico: `BANCOS_SOLO_CON_MODO`). Una promo "X Modo" aplica SOLO a quien marca "X Modo" — no a quien tiene solo MODO ni solo el banco. MODO genérico sigue siendo su opción. Se detecta: (a) por el nombre de la entidad (`resolverTarjetasDeNombre` → banco + MODO → `combinarConModo`): "ICBC Modo", "Hipotecario_Modo", "Yoy_MODO", "BNA+MODO"; (b) MODO y un banco en la MISMA entrada (Día "Modo|BNA" → Banco Nación Modo); (c) Coto: descripción + ícono ("Pagando con MODO desde la app de Comafi"); (d) texto corto que exige MODO (`textoExigeModo`): Carrefour "Exclusivo pagando con Modo" (BNA 5% jubilados), Cencosud `info` "Aplica sólo a pagos realizados a través de QR de Modo desde app Galicia y app Modo" (Galicia 20%/30%, Supervielle, Banco Ciudad) y "Desde App Hipotecario o App Modo" — estas de Cencosud antes se aplicaban a cualquiera con la tarjeta. Frases con "acumulable"/"no aplica" no cuentan. Un "<banco sin canónico> MODO" ("Banco San Juan MODO", "Semana_MODO") se descarta. El orden de `TARJETAS_CONOCIDAS` pone cada "X Modo" justo después de su banco.
- **Requisitos = segmentos del cliente + programas/tiendas** (`SEGMENTOS` = Jubilado, Plan sueldo, Empleado público, Supervielle Identité; `PROGRAMAS` = MasGO, Comunidad Coto), opciones propias al final de Mis descuentos. `requisitosDeTexto()` los detecta SOLO en los campos cortos (nombre de entidad, título/sub_title en Carrefour/Chango Más, `info` en Cencosud, título + encabezado corto del legal en Día, `descripcion` + ícono en Coto), nunca en el legal completo. Jubilado = jubilados, pensionados, beneficiarios ANSES, mayores de 60. Dentro de cada frase (corte en ". " que no sea "$15.000", "!", "|", " - "), una mención no cuenta si hay un "tope" antes ("Tope mensual: $10.000 para Cartera General y $30.000 para … Sueldo … y Jubilado", Hipotecario_Modo), si le sigue "tope" ("Si sos jubilado tope de $5.000", Cuenta DNI) o si la frase dice "adicional" ("Y si sos jubilado, tenés 5% adicional"). Tres casos: banco + segmento (Banco Nación Modo 5% jubilados en Carrefour y Día, Supervielle "Exclusivo Jubilados"/"MARTES JUBILADOS", Mi Carrefour 10% ANSES/+60, ICBC_Sueldos y Coto ICBC "PLAN SUELDO", Coto Supervielle "SGTO IDENTITÉ", MasClub/Banco Ciudad/MODO "Exclusivo MasGo") → tarjeta Y requisito; solo segmento, sin banco (`clasificarPromo`: entidad "Anses"/"Beneficios Anses"/"Empleados Públicos"/"Medios de Pago" o sin entidad; Coto por `ICONO_REQUISITO_COTO`: comunidad, jubilados y pensionados, beneficios ANSES) → solo el requisito; promo general del banco → no se restringe. Un banco que la app no modela con texto de jubilados se sigue descartando (no se regala a todos los jubilados). Antes varias se descartaban (Anses, Empleados Públicos, Medios de Pago jubilados de Vea/Jumbo/Disco, íconos de Coto) y otras se aplicaban a todos (Supervielle jubilados, Mi Carrefour ANSES, ICBC Sueldos, las MasGO).
...
- **Grilla**: logo = la tarjeta ("ICBC Modo" → logo de ICBC en `LogoBanco.tsx`); sin tarjeta, el requisito o "Todos los medios de pago" (iniciales). Prioridad por banco y por celda: primero las que el usuario puede usar; entre las demás, las sin requisito antes que las con requisito (a quien no marcó MasGO no le aparece "MasClub 20%" de MasGO tapando el 15% general); después, mayor %.
- **Mis descuentos**: cada opción lista las promos que la nombran como tarjeta o como requisito ("ICBC Modo" ya no aparece bajo "MODO" ni bajo "ICBC"). La pantalla no agrupa (lista plana + buscador): los "<Banco> Modo" van junto a su banco y los segmentos/programas al final.
...
**Selección de tarjetas del usuario — una sola fuente de verdad.** No existe una lista de tarjetas separada por pantalla: `carrito.tarjetas` (contexto `ProveedorCarrito`/`useCarrito` en `app/src/carrito.tsx`, campo persistido en AsyncStorage bajo `allpromos:carrito:v1` junto con los ítems del carrito) es leída y escrita tanto por el bloque "Mis descuentos" del carrito (`app/app/(tabs)/carrito.tsx`, solo lectura ahí — tocarlo navega a `/mis-descuentos`) como por los switches de `app/app/mis-descuentos.tsx` (`carrito.setTarjetas(...)`). `TARJETAS_DISPONIBLES` en `carrito.tsx` (43 entradas desde 2026-09-24: se sumaron los 11 "<Banco> Modo" nuevos y los 6 requisitos — Jubilado, Plan sueldo, Empleado público, Supervielle Identité, MasGO, Comunidad Coto —; un test compara sus strings con `TARJETAS_CONOCIDAS`. Lista anterior de 26 — Banco Hipotecario sumado 2026-09-24 —: Mi Carrefour, Cuenta Digital Carrefour, Tarjeta Carrefour Crédito, MasClub, Cencopay, Santander, MODO, Mercado Pago, Cuenta DNI, Banco Provincia, Galicia, Galicia Modo, Banco Macro, HSBC, BBVA, ICBC, Comafi, Naranja X, Credicoop, Banco Ciudad, Supervielle, Banco Columbia, Banco Patagonia, Banco Nación, TCI — los 3 beneficios propios de Carrefour separados 2026-09-04, ver "Corregido 2026-09-04" arriba; los 9 bancos de Cencosud sumados 2026-08-26, ver quirk de Coto/icono de banco más arriba) coincide 1 a 1 con `ALIAS_TARJETAS`/`TARJETAS_CONOCIDAS` de `promos-bancarias.js` — mismos strings canónicos, sin mapeo de por medio. Si se agrega un banco nuevo a `ALIAS_TARJETAS`, hay que sumarlo también acá a mano (no se deriva automáticamente); `mis-descuentos.tsx`, en cambio, sí deriva de `TARJETAS_CONOCIDAS` sin necesidad de tocarlo. Ese array llega tal cual al backend en cada `POST /api/comparar` como `tarjetas`; el backend nunca lee `mis-tarjetas.json` (eso es solo del CLI, ver `leerMisTarjetas()` más abajo).
