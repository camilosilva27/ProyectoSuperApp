...
- [x] Promos de segmento (jubilados/ANSES/+60, plan sueldo, empleados públicos, Supervielle Identité, Mi Carrefour 10% ANSES/+60) → requisitos propios en Mis descuentos (tarjeta Y segmento, o solo segmento). Y MODO + banco → el banco con `viaModo` (aplica con solo el banco marcado, no con solo MODO; etiqueta "ICBC vía MODO"; reemplaza `requiereTodas` y el canónico propio "<Banco> Modo" que existió unas horas; opciones viejas "<Banco> Modo" guardadas se normalizan a banco + MODO). Ver "Modelo de tarjetas y requisitos" en CONTEXTO_TECNICO.md.
...
- [x] ✅ RESUELTO 24-25/09. Promos bancarias limitadas a categorías, con exclusiones y "no acumulables" → el % va solo sobre las filas del carrito que cumplen (`categoriasIncluidas`/`categoriasExcluidas`/`marcasExcluidas`/`soloSinOferta`, vocabulario en `AllPromos/core/categoriasPromo.js`; categoría por EAN y súper vía `precioCache.categoriasPorEAN`). Afectadas hoy: Cencopay 25% jueves (Vea: galletitas/golosinas/conservas de frutas y verduras/cervezas; Jumbo/Disco: galletitas/bebidas sin alcohol/perfumería/limpieza), Cencopay 40% vie-dom (+ solo local), Carrefour Empleado público mié/jue (almacén, congelados, lácteos, fiambres, bebidas, limpieza, perfumería), Chango Más empleados municipales/provinciales; exclusiones de carnicería/huevos/frutas y verduras/leches/electro/marcas (Quilmes, línea Coca-Cola, bodegas, marca propia Carrefour) en casi todas; "no acumulable" en Cencopay, varias de Carrefour y casi todas las de Chango Más. Chango Más ANSES 10% ("únicamente en los productos detallados…", lista no mapeable) se DESCARTA por la regla de seguridad. Ejemplo real: Jumbo jueves, galletitas $3.900 + milanesa $18.999 + Coca Zero $5.900 → Cencopay 25% antes $7.199,75, ahora $2.450. App: "· solo en …" en la fila del Plan de Compra y "%*" + nota en la grilla. Ver "Promos limitadas a categorías" en CONTEXTO_TECNICO.md.
