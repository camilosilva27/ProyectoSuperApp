/**
 * Scraper de Disco — guarda TODO el catálogo con EAN y marca promos activas.
 *
 ...
 *
 * Salida:
 *   catalogo-disco.json  — todos los SKUs con EAN, precio y promo (null si no tiene)
 *   promos-disco.json    — solo los SKUs con promo activa (para búsqueda rápida)
 */
...
async function main() {
  console.log('=== Scraper Disco — Catálogo completo + Promociones ===\n');
  ...
  fs.writeFileSync('./catalogo-disco.json', JSON.stringify({
    ...meta,
    total_skus: catalogo.length,
    skus: catalogo,
  }, null, 2));

  fs.writeFileSync('./promos-disco.json', JSON.stringify({
    ...meta,
  ...
  console.log(`  Guardado en:     catalogo-disco.json + promos-disco.json`);
  console.log('\nTop 10 descuentos:');
