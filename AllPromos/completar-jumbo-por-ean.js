/**
 * Completa catalogo-jumbo.json con productos que Jumbo sí vende pero el scraper normal
 * (scraper-promos-jumbo.js) no trajo, por el techo de ~2.550 SKUs del endpoint de paginación
 * masiva de VTEX.
 *
 ...
 * embebidas como Carrefour/Chango Más/Día), porque Jumbo comparte la MISMA cuenta VTEX que Vea
 * (mismo EAN/master data, ver scraper-promos-jumbo.js). Aun así, cada sitio pagina su propio
 * top ~2.550 por relevancia/ventas DE ESE SITIO, así que el recorte de Jumbo puede diferir del
 * de Vea — por eso vale la pena cruzar Jumbo también contra Vea y Disco, no solo contra
 ...
 * igual que el scraper normal de Jumbo (no hizo falta para Jumbo/Disco, a diferencia de Vea).
 *
 * Salida: NUNCA toca catalogo-jumbo.json (eso es 100% del scraper normal, se resetea cada
 * corrida). Escribe/actualiza catalogo-jumbo-extras.json — solo lo que aportó este script — y
 * promos-jumbo-extras.json. `core/catalogo.js` (leerCatalogo) mezcla base+extras de forma
...
function candidatosDeEAN(eansYaEnJumbo) {
  ...
  for (const archivo of FUENTES_CANDIDATAS) {
    ...
    for (const s of skus) {
      ...
      if (eansYaEnJumbo.has(ean)) continue;
...
const NOMBRE_CHECKPOINT = 'jumbo';

async function main() {
  console.log('=== Completar catálogo de Jumbo por EAN (huecos del techo de 2.550) ===\n');

  const actual = leerCatalogo('catalogo-jumbo.json');
  const skusActuales = Array.isArray(actual?.skus) ? actual.skus : [];
  const eansYaEnJumbo = new Set(skusActuales.filter(s => s.ean).map(s => String(s.ean)));
  console.log(`Catálogo actual de Jumbo: ${skusActuales.length} SKUs (${eansYaEnJumbo.size} EAN únicos)`);

  let candidatos = candidatosDeEAN(eansYaEnJumbo);
  ...
  console.log(`EAN candidatos a probar (de Vea/Carrefour/Chango Más/Día/Coto/Disco, fuera del catálogo actual): ${candidatos.length}${argLimit ? ' (recortado por --limit)' : ''}\n`);
  ...
  console.log(`✅ Nuevos SKUs encontrados en Jumbo: ${encontrados.length}`);
  console.log(`   Negativos: ${negativos.length}`);
  console.log(`   Errores de red: ${erroresRed}`);

  if (!encontrados.length) {
    console.log('\nNada para agregar — no se toca catalogo-jumbo-extras.json.');
  ...
  // Extras existentes (archivo aparte del catalogo-jumbo.json que reescribe el scraper normal
  // — ver completador_catalogos.md § 6): se le suman los nuevos hits de esta corrida.
  const RUTA_EXTRAS = './catalogo-jumbo-extras.json';
  let extrasActuales = [];
  try {
  ...
  console.log(`  Guardado en:                     catalogo-jumbo-extras.json + promos-jumbo-extras.json`);
  borrarCheckpoint(NOMBRE_CHECKPOINT);
}
