/**
 * Refresca precio+promo de los productos que YA están en catalogo-jumbo-extras.json (los que
 * encontró completar-jumbo-por-ean.js), SIN volver a buscar candidatos nuevos.
 * Ver refrescar-precio-extras-carrefour.js (por qué) y refrescar-precio-extras-vea.js (promo
 * aparte, mismo esquema que Jumbo por compartir cuenta VTEX) para el detalle completo.
 *
 * Uso: node refrescar-precio-extras-jumbo.js
 */
...
/** Mismo parseo de precio que buscarPorEAN() en completar-jumbo-por-ean.js, filtrando por el
...
async function main() {
  console.log('=== Refrescar precio de extras de Jumbo (sin buscar candidatos nuevos) ===\n');

  let data;
  try {
    data = JSON.parse(fs.readFileSync(RUTA_EXTRAS, 'utf8'));
  } catch {
    console.log(`No existe ${RUTA_EXTRAS} todavía — nada para refrescar. Corré primero completar-jumbo-por-ean.js.`);
    return;
  }
  ...
  console.log(`  Guardado en:     ${RUTA_EXTRAS} + promos-jumbo-extras.json`);
}
