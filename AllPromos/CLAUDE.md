...
- **Comparación generalizada a N supers vía el array `SUPERMERCADOS`** en `buscar-promos.js` (no hardcodeada a 2 ni a 3). Un 4to super se suma agregando una entrada ahí + sus funciones `xLive*`/`parsearProductosX`; no tocar `mostrarComparativo`/`mostrarResumenFinal` para eso.
