...
- **Promos bancarias excluidas** (Tarjeta, Cuenta Digital, Banco, "BIN" como palabra suelta) — un solo criterio: `esTeaserBancario()` en `promo-engine.js`, usado por scrapers, completadores, refrescadores, `core/fetchers.js` y `backend/src/precioCache.js` (que además reclasifica al leer catálogos viejos). Nunca usar `includes('bin')`: "Com**bin**able" lo contiene y le sacaba el 2do al X%/NxM a ~561 SKUs de Carrefour (bug corregido 2026-09-24). Si agregás una fuente de promos nueva, usar el helper.
