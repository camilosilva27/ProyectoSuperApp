/**
 * Refresca precio+promo de los productos que YA están en catalogo-vea-extras.json (los que
 * encontró completar-vea-por-ean.js), SIN volver a buscar candidatos nuevos.
 * Ver refrescar-precio-extras-carrefour.js para el porqué completo — mismo patrón acá, pero
 * la promo se pide aparte (mismo endpoint `_v/search-promotions` que ya usa
 * completar-vea-por-ean.js) porque Vea no la trae embebida en el precio como Carrefour.
 *
 * Uso: node refrescar-precio-extras-vea.js
 */
...
function postJSON(url, body, retries = 3) {
  return new Promise((resolve, reject) => {
    ...
    // ese lote sin promo esta corrida en vez de tirar abajo todo el refresco de precios.
    req.on('error', () => {
      if (retries > 0) {
        sleep(10000).then(() => resolve(postJSON(url, body, retries - 1)));
        return;
...
/** Mismo parseo de precio que buscarPorEAN() en completar-vea-por-ean.js, filtrando por el
 *  lote de skuId pedido en vez de por un único EAN objetivo. */
...
async function main() {
  console.log('=== Refrescar precio de extras de Vea (sin buscar candidatos nuevos) ===\n');

  let data;
  try {
    data = JSON.parse(fs.readFileSync(RUTA_EXTRAS, 'utf8'));
  } catch {
    console.log(`No existe ${RUTA_EXTRAS} todavía — nada para refrescar. Corré primero completar-vea-por-ean.js.`);
    return;
  }
  ...
  escribirAtomico('./promos-vea-extras.json', JSON.stringify({
    ...meta,
  ...
  console.log(`  Guardado en:     ${RUTA_EXTRAS} + promos-vea-extras.json`);
}
