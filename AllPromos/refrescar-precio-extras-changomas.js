/**
 * Refresca precio+promo de los productos que YA están en catalogo-changomas-extras.json (los
 * que encontró completar-changomas-por-ean.js), SIN volver a buscar candidatos nuevos.
 * Ver refrescar-precio-extras-carrefour.js para el porqué completo — mismo patrón acá.
 *
 * Uso: node refrescar-precio-extras-changomas.js
 */
...
// Copiado sin cambios de completar-changomas-por-ean.js.
...
/** Mismo parseo que buscarPorEAN() en completar-changomas-por-ean.js, filtrando por el lote de
 *  skuId pedido en vez de por un único EAN objetivo. */
...
async function main() {
  console.log('=== Refrescar precio de extras de Chango Más (sin buscar candidatos nuevos) ===\n');

  let data;
  try {
    data = JSON.parse(fs.readFileSync(RUTA_EXTRAS, 'utf8'));
  } catch {
    console.log(`No existe ${RUTA_EXTRAS} todavía — nada para refrescar. Corré primero completar-changomas-por-ean.js.`);
    return;
  }
  ...
  fs.writeFileSync('./promos-changomas-extras.json', JSON.stringify({
    ...meta,
  ...
  console.log(`  Guardado en:     ${RUTA_EXTRAS} + promos-changomas-extras.json`);
}
