...
**Código commiteado, deployado a la VM y con credenciales de Test cargadas (2026-08-21)** — confirmado con `curl` contra `https://34.24.10.174.nip.io`: `/api/pagos/suscripcion` y `/api/webhooks/mercadopago` responden 401 (rechazan sesión/firma inválida) en vez de 503 (no configurado), y `/api/health` sigue en 200 — el resto del backend no se vio afectado. Cuenta usada: la personal del usuario, sin monotributo (ver decisión más abajo). Falta correr las migraciones `0004`/`0005`/`0006` en el proyecto Supabase real (siguen sin correr) y probar el flujo completo con los usuarios de prueba de MP ya creados.
...
- **Fase B ya implementada y migrada al proyecto real** (2026-08-21): `app/src/historialAhorro.tsx` sincroniza con Supabase cuando hay sesión, mismo patrón row-based que `carritosGuardados.tsx` (no `useSincronizacionPersistente`, porque es un log que solo crece, no un blob). Tabla nueva `ahorro_registro` (`id`, `usuario_id`, `fecha`, `monto` — sin columna `super`, porque `EventoAhorro` nunca la tuvo en la implementación real de Fase A) con RLS de select/insert propio, en `supabase/migrations/0007_ahorro_registro.sql`, **corrida contra el proyecto Supabase real** (`supabase db push`, confirmado con `supabase migration list`: 0001-0007 remoto=local). Primer login con historial local y servidor vacío sube todos los eventos locales como filas nuevas; si el servidor ya tenía filas, esas ganan y el local se descarta — mismo criterio que `carrito_guardado` (Fase 1). Sin verificar todavía en vivo con una cuenta real (`tsc --noEmit` limpio, no probado en navegador).
