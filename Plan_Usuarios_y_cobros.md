...
- **Fase C ya implementada** (2026-08-21, corregida el mismo día — la primera versión de este punto describía un alcance que el usuario amplió después): `app/src/componentes/PaywallFinTrial.tsx` quedó cableada como gate real vía `app/src/componentes/GatePaywallFinTrial.tsx` (montado en `_layout.tsx`, adentro de `GateSesion`, envolviendo el `Stack`). Fase B (migrar `historialAhorro` a una tabla Supabase) sigue sin implementar — el ahorro mostrado en el paywall sigue siendo el tracking local (`AsyncStorage`), no un dato server-side.
