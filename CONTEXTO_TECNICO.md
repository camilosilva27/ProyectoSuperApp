...
- **4 de los 5 catálogos locales capturan solo una fracción del catálogo real** (tope de ~2.550 ítems del endpoint legacy de VTEX — Vea, Carrefour, Chango Más y Día). Coto es la excepción: no tiene ese tope (Constructor.io pagina distinto) y captura sus ~57.623 SKUs reales completos. La búsqueda por nombre puede no encontrar productos poco comunes que no entraron en ese recorte en los otros 4.
  ...
  **Investigado a fondo el 2026-08-19 (replanteado: no es "migrar de API", ver "Pendientes" abajo):**
  ...
  5. **El verdadero riesgo no es el scraping (corre en un cron cada 2hs, nadie espera eso) — es la búsqueda en vivo de la app.** `backend/src/catalogoUnificado.js` hace un filtro LINEAL en memoria sobre TODO el catálogo unificado en cada request de `/api/catalogo/buscar` (se dispara en cada tecla que escribe cualquier usuario, con 300ms de debounce) — el propio comentario del archivo ("a este volumen ~5-7k productos un filtro lineal alcanza") ya está desactualizado: hoy el catálogo unificado real ya es ~60.611 productos (por el catálogo completo de Coto), y una cobertura completa de los 4 restantes lo llevaría a un estimado de ~500-600 mil. **Medido en vivo con el código real (`catalogoUnificado.buscar()`) contra un catálogo sintético de 606.110 productos** (10x el actual, réplica del catálogo real de producción): cada búsqueda pasa de tardar ~55-220ms (hoy, a 60k) a **~270-315ms constantes, sin importar la especificidad de la query** — el costo lo domina escanear las 606k entradas, no filtrar los resultados. La carga inicial del archivo (una vez por cambio del cron) pasa de ser instantánea a **~1,5 segundos** de bloqueo del proceso. Esto corrió en una Mac; en la VM real (e2-micro, CPU compartida/débil) sería peor. Como el escaneo es síncrono, bloquea el único hilo de Node — mientras resuelve la búsqueda de un usuario, el server no puede atender NINGUNA otra request (ni de otro usuario, ni `/api/comparar`) hasta terminar.
  6. **Conclusión: no se puede escalar la cobertura del catálogo sin antes reemplazar el filtro lineal por algo indexado** (índice invertido por palabra, o mover a algo como SQLite FTS) — es una tercera pieza de trabajo, separada de "partir el scraping por categoría" y de "elegir qué API usar". Sin eso, ampliar cobertura mejoraría la búsqueda de productos raros pero degradaría la experiencia de búsqueda para TODOS los usuarios, todo el tiempo.
...
- **Replanteado (2026-08-19): cobertura completa de catálogo (Vea/Carrefour/Chango Más) no es "migrar de API", son tres tareas separadas** — ver la investigación detallada en "Alcance y limitaciones" arriba: (1) partir el scraping por categoría (la Intelligent Search de VTEX ayuda poco: techo de 5.000/consulta vs. 2.550 legacy, sigue siendo por consulta, ejemplo medido en Vea: hasta 458 categorías-hoja necesarias), (2) reemplazar el filtro lineal de `catalogoUnificado.js` por un índice antes de escalar el catálogo 10x, o cada búsqueda de cualquier usuario pasaría de ~100ms a ~300ms+ bloqueando el server entero (medido con el código real contra un catálogo sintético de 606k), (3) confirmar que la Intelligent Search no tenga rate-limiting propio en Carrefour/Chango Más (solo se probó en Día). La (2) es bloqueante: sin eso, ampliar cobertura degrada la app para todos los usuarios a cambio de mejorar la búsqueda de productos raros.
