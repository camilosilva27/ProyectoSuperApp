...
- **Replanteado y ampliado (2026-08-19): cobertura completa de catálogo (Vea/Carrefour/Chango Más) no es "migrar de API", son varias tareas separadas, y una de ellas bloquea todo el resto** — ver la investigación detallada en "Alcance y limitaciones" arriba: (1) partir el scraping por categoría (la Intelligent Search de VTEX ayuda poco: techo de 5.000/consulta vs. 2.550 legacy, sigue siendo por consulta, ejemplo medido en Vea: hasta 458 categorías-hoja necesarias), (2) confirmar que la Intelligent Search no tenga rate-limiting propio en Carrefour/Chango Más (solo se probó en Día, sin problemas), (3) **bloqueante real: la VM de producción (e2-micro, 958 MB de RAM) no tiene memoria para sostener un catálogo ~10x más grande — probado en la VM real, el proceso de Node crashea con out-of-memory al intentar cargarlo** (hoy ya usa 293 MB de swap a la escala actual). Ni siquiera llega a ser "lento": el dato no entra. (4) el filtro lineal de `catalogoUnificado.js` tampoco escalaría bien aunque hubiera RAM (cada búsqueda pasaría de ~50-200ms a ~300ms fijos, bloqueando el único hilo de Node para todos los usuarios). Dato a favor: los ~2.550 que se capturan hoy no son arbitrarios, son los más vendidos de cada super (confirmado contra `O=OrderByTopSaleDESC`) — lo que falta cubrir es la cola larga, no productos populares.
